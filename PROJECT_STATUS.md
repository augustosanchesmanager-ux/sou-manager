...
6.0.5  Feature Flags      ████████████████████░░  80%  ✅ 6.0.5.1 + 6.0.5.2 CERTIFICADAS (smoke 10/10; deploy deferido) + 6.0.5.3 implementada (unit 847/847, smoke 10/10) + 6.0.5.4 implementada (unit 874/874, migration T1–T7 OK; flow14 E2E adiado p/ janela única — decisão PO) + 6.0.5.5 IMPLEMENTADA (unit 883/883, migration `20260807020000` T1–T12 OK em docker, **SCHEMA FREEZE = YES**; E2E flow11 adiado — ver `PHASE_6_0_5_5_ENTRY_AUDIT.md` §12) + **Hardening RPCs irmãs ✅ (migration `20260808000000` — S1–S16 + G1 PASS, decisão PO D-6.0.5.5-6..8)**
...
| SaaS Core Implementation | 6.0 | 🔵 6.0.1 + 6.0.2 + 6.0.3 concluídas — 6.0.4.1 + 6.0.4.2 + 6.0.4.3 concluídas (hardening RPC + lifecycle) — **6.0.4.4 Billing Engine CONCLUÍDA (baseline v1.4.2, E2E flow9/flow12 verdes)** — **6.0.5.1 CERTIFICADA (baseline v1.4.3, E2E flow13 8/8)** — **6.0.5.2 CERTIFICADA (smoke 10/10, 819 testes, deploy deferido pelo PO)** — **6.0.5.3 implementada (commit `b383222`, smoke 10/10, deploy deferido)** — **6.0.5.4 implementada (unit 874/874, migration `20260807010000` T1–T7 OK em docker; E2E flow14 adiado p/ janela única)** — **6.0.5.5 IMPLEMENTADA (unit 883/883, migration `20260807020000` T1–T12 OK em docker — RPC `change_tenant_plan` superadmin + espelho `tenants.plan` + `changePlan`/`TenantSubscriptionUpdated` + `Admin.tsx` sem escrita direta + `UpgradePrompt` + `StatusBanner` + depreciação `featureAvailability.ts`; **SCHEMA FREEZE = YES**; **Hardening de RPCs irmãs ✅** — auditoria de estado efetivo + validação PG16 (S1–S16 + G1) encontrou `create_invoice`/`record_payment_attempt` quebradas (declaradas "limpas" incorretamente) e corrigiu na migration `20260808000000` (aditiva, idempotente 2×; sem mudança de regra/contrato — D-6.0.5.5-6..8); E2E flow11/flow14 adiados à janela única — ver `PHASE_6_0_5_5_ENTRY_AUDIT.md` §12)** — **6.0.5.6 PCA EXECUTADA → ❌ BLOCKED (auditoria somente leitura do banco real — ver `PRODUCTION_COMPATIBILITY_AUDIT.md`; migration `20260806030000` pulada no remoto com `cancel_subscription` incompatível — 11 colunas aplicadas × 5 pendentes — erro garantido na janela; correção aguardando decisão do PO; 3 tenants excedem `max_staff=1` do plano `free`: Barbearia Principal produtivo, Loja Demo Varejo, SMG Estética)** — **6.0.6 Compliance & Legal registrada (gate obrigatório de certificação da release v1.5 — decisão PO 2026-08-07; entry audit `PHASE_6_0_6_ENTRY_AUDIT.md` criada)** |
...
|------|--------|-----------|
| 2026-08-08 | 6.0 | **6.0.5.6 — Production Compatibility Audit (PCA) EXECUTADA → ❌ `BLOCKED` (2026-08-08, somente leitura).** Auditoria do banco real dos tenants produtivos (project ref `ushsnmlbeurfvlkieiln` — Sanchez Barber) concluída com **1 incompatibilidade crítica de migration + 3 excedências de limite**. **Bloqueio crítico:** a migration `20260806030000` foi **pulada no remoto** (topologia conhecida) — o `cancel_subscription` atual (11 colunas com `cancel_at_period_end`, definido pelas migrations `06050000`/`06070000` **já aplicadas**) é **incompatível** com o `CREATE OR REPLACE` de 5 colunas da pendente → PostgreSQL rejeita (`cannot change return type of existing function`), **erro garantido na janela única**; além disso a `06030000` regrediria a semântica de cancelamento (pedido → efetivo) do BillingEngine 6.0.4.4. A autorização que ela adiciona **já está no remoto via `06070000`** (`current_is_tenant_manager_from_auth_uid`). **Recomendação: `supabase migration repair --status applied 20260806030000` (decisão do PO) ou migration corretiva.** **Dados:** Barbearia Principal (produtivo, `free`, **4 staff** ativos), Loja Demo Varejo (`free`, 3) e SMG Estética (`free`, 2) **excedem `max_staff=1`** — decisão de negócio do PO. **Compatível:** tenants 0 sem plano / 0 planos inválidos / status todos no enum; 0 subscriptions órfãs; invoices 0 / billing_events 2 / payment_attempts 0 (0 órfãos); 15 assinaturas Chef Club + 42 receivables íntegras; 16 funções auditadas com assinaturas compatíveis; FKs íntegras. Veredito detalhado em `docs/audit/PRODUCTION_COMPATIBILITY_AUDIT.md`. **Gate de release mantido: nenhuma migration de produção sem PCA = READY.** |
