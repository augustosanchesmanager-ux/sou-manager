...
6.0.5  Feature Flags      ████████████████████░░  80%  ✅ 6.0.5.1 + 6.0.5.2 CERTIFICADAS (smoke 10/10; deploy deferido) + 6.0.5.3 implementada (unit 847/847, smoke 10/10) + 6.0.5.4 implementada (unit 874/874, migration T1–T7 OK; flow14 E2E adiado p/ janela única — decisão PO) + 6.0.5.5 IMPLEMENTADA (unit 883/883, migration `20260807020000` T1–T12 OK em docker, **SCHEMA FREEZE = YES**; E2E flow11 adiado — ver `PHASE_6_0_5_5_ENTRY_AUDIT.md` §12)
...
Última auditoria: 08/08/2026 (6.0.5.5 — IMPLEMENTADA: migration `20260807020000` validada em docker T1–T12, unit 883/883, SCHEMA FREEZE = YES; ⚠️ descoberta: RPCs irmãs 6.0.4/6.0.5.4 com bug latente de referência ambígua — nunca executadas em Postgres real)
```
...
| SaaS Core Implementation | 6.0 | 🔵 6.0.1 + 6.0.2 + 6.0.3 concluídas — 6.0.4.1 + 6.0.4.2 + 6.0.4.3 concluídas (hardening RPC + lifecycle) — **6.0.4.4 Billing Engine CONCLUÍDA (baseline v1.4.2, E2E flow9/flow12 verdes)** — **6.0.5.1 CERTIFICADA (baseline v1.4.3, E2E flow13 8/8)** — **6.0.5.2 CERTIFICADA (smoke 10/10, 819 testes, deploy deferido pelo PO)** — **6.0.5.3 implementada (commit `b383222`, smoke 10/10, deploy deferido)** — **6.0.5.4 implementada (unit 874/874, migration `20260807010000` T1–T7 OK em docker; E2E flow14 adiado p/ janela única)** — **6.0.5.5 IMPLEMENTADA (unit 883/883, migration `20260807020000` T1–T12 OK em docker — RPC `change_tenant_plan` superadmin + espelho `tenants.plan` + `changePlan`/`TenantSubscriptionUpdated` + `Admin.tsx` sem escrita direta + `UpgradePrompt` + `StatusBanner` + depreciação `featureAvailability.ts`; **SCHEMA FREEZE = YES**; ⚠️ descoberta de bug latente nas RPCs irmãs 6.0.4/6.0.5.4 — ver entry audit §12.5; E2E flow11/flow14 adiados à janela única — ver `PHASE_6_0_5_5_ENTRY_AUDIT.md` §12)** — **6.0.6 Compliance & Legal registrada (gate obrigatório de certificação da release v1.5 — decisão PO 2026-08-07; entry audit `PHASE_6_0_6_ENTRY_AUDIT.md` criada)** |
