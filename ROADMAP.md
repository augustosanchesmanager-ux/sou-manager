...
| D7 | Transactional Outbox (atomicidade RPC + enqueue) | Trilha C (2026-08-26) instalou Durable Outbox (`SupabaseOutbox` — `outbox_items` com `FOR UPDATE SKIP LOCKED`, stale recovery, idempotência em 3 camadas). Porém, **a atomicidade entre `finance_settle_comanda` RPC e `outbox.enqueue()` não foi resolvida** — a RPC commita independentemente antes do evento ser publicado. Se o enqueue falhar após o commit da RPC, o evento é perdido (durable ≠ transactional). **Impacto:** perda silenciosa de commission_records em cenário de falha no enqueue pós-commit. **Mitigação atual:** Durable Outbox com retry + stale recovery reduz a janela, mas não elimina o gap. **Solução requer:** ADR separado para Transactional Outbox (pattern: change-data-capture, dual-write com polling, ou TX-based outbox) | Certificação (2026-08-26) | ⬜ Registrada — Trilha C certificada (cf451be); ADR pendente |
