...
|------|--------|-----------|
| 2026-09-02 | 8.43 | **H-8 (Homologação) 🟢 PASS COM CONDIÇÃO → PRODUÇÃO APLICADA — retrofit ACL das 3 RPCs (D-HOM-28 + D-HOM-29, 2026-09-02).** Revalidação read-only da produção `ushsnmlbeurfvlkieiln`: premissa D-HOM-10 obsoleta — `smg-barber` em `c1c40da` (FASE 6); 4 migrations corretivas verificadas. Condição: ACL residual em 3 RPCs. Staging homologado (`tjcvuhynckocmvtqykxp`): migration aplicada, 3/3 PASS. **Produção aplicada** (`ushsnmlbeurfvlkieiln`): PO autorizou; `confirm_appointment_attendance`, `correct_appointment_attendance`, `register_comanda_payment` — `anon`/`service_role` revogados, `authenticated` mantido. **H-8彻底 encerrado.** `get_comanda_payment_summary` residual fora do escopo (trilha futura). Próximo bloqueador: H-7 (S3-1 R$260 RIOS + janela acompanhada). Registros: `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-28/29), `docs/BUSINESS_DECISIONS.md` (D-HOM-28/29), `PROJECT_STATUS.md` (7.4/7.5). |
