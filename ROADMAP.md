...
|------|--------|-----------|
| 2026-09-26 | 8.79 | **H-8 — 🟢 CERTIFICADO/HOMOLOGADO — formalização documental da certificação (frente docs-only, autorização do PO; D-HOM-33).** **Gate de certificação do H-8 executado em 2026-09-26 (read-only, 6 etapas, zero alterações):** ETAPA 1 identidade (PR #88 `MERGED` `1a73156`, tree local = main, CI obrigatória ✅, lint = baseline) · ETAPA 2 deploy (GitHub Deployments 3/3 `success` no sha `1a73156`; `vercel inspect` → `dpl_42QfhwvzccUi5MxDQuvfVsjJEtzf` Ready/production alias `barber.soumanager.com`; bundle = `ushsnmlbeurfvlkieiln` + anon key local) · ETAPA 3 cenários **H8-1..H8-7 ✅ 7/7** — **H8-4/H8-6 REVALIDADOS via API Vercel** (fecha a ressalva do D-HOM-32: oficial só 4 chaves públicas/client; `MULTI_SCHEMA_ENABLED` ausente × legado presente = pendência PO) · ETAPA 4 segurança (clients 5/5 tenant `b716e290`, `tenants` row única, anon `[]`/`42501`, RPC manager/active) · ETAPA 5 regressão: **nenhuma** (boot/login/assets/dados + e2e smoke ✅ CI) · ETAPA 6 veredito **A) CERTIFICÁVEL** → **PO declarou H-8 HOMOLOGADO/CERTIFICADO**. **Escopo desta frente:** docs-only em 4 arquivos (`docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md`, `docs/BUSINESS_DECISIONS.md`, `ROADMAP.md`, `PROJECT_STATUS.md`); **NÃO autoriza** pendências (`MULTI_SCHEMA_ENABLED`, projeto legado, bypass staging, P1-01, `SEP-IDENTITY-CONTRACT`, lint, exclusão de branches — cada uma com gate próprio). **STOP:** PR aguarda revisão/`/approve` do PO — merge NÃO autorizado nesta sequência. Registros: `PROJECT_STATUS.md` (7.40), `docs/BUSINESS_DECISIONS.md` (D-HOM-33), `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (STATUS/§8.2) |
| 2026-09-26 | 8.78 | **H-8 — 🟡 DESBLOQUEADO TECNICAMENTE COM RESSALVAS — formalização documental do veredito (frente A-H8 docs-only, autorização do PO; D-HOM-32; gate B+C anterior concluído).** Veredito formal do PO sobre o gate H-8 após a auditoria **B+C read-only** de 2026-09-26: **H8-1 topologia · H8-2 domínio oficial→`ushsnmlbeurfvlkieiln` · H8-3 sem double-deploy · H8-5 config build · H8-7 produção atualizada (deploy `6678953234` = `9aed924`, success; premissa `718f6f9` superada; 6/6 deployments success) ✅**; **H8-4/H8-6 ⚠️ RESSALVAS METODOLÓGICAS** (não revalidados via API Vercel na B+C — escopo HTTP/GoTrue/RLS/REST; baseline 2026-08-08 mantida; sem sintoma funcional; não reabrir por esta razão). Evidências B+C: login GoTrue 200, RPC context `b716e290`/manager/active, anon 401/42501, RLS sem vazamento, reversão SEP leitura OK (`professional_id` → `42703` = drift PROD conhecido, sem mutação). **Distinção vinculante (PO): "desbloqueado tecnicamente" NÃO equivale à certificação da release** — certificação/homologação permanece gate separado (sequência A-H8 → STOP/revisão PO → certificação → STOP → P1-01). **Escopo desta frente:** docs-only em 4 arquivos (`docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md`, `docs/BUSINESS_DECISIONS.md`, `ROADMAP.md`, `PROJECT_STATUS.md`); **proibido:** código, SQL, migration, RPC, RLS, alteração Vercel/config, novo smoke, mutação financeira, certificação antecipada, P1-01. **STOP:** PR aguarda revisão/`/approve` do PO — merge NÃO autorizado nesta sequência. Registros: `PROJECT_STATUS.md` (7.39), `docs/BUSINESS_DECISIONS.md` (D-HOM-32), `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (STATUS/§8/§8.1). |
| 2026-09-26 | 8.77 | **DRIFT-FIX-REV-SEP-01 🟢 ENCERRADA (fix `reversal.ts`) + H2-8 config 🟢 ENCERRADA (versionamento do harness) + Produção/Deployment 🟢 AUDITADO read-only — formalização documental do ciclo 2026-09-26 (frente A docs-only, autorização do PO).** **Drift comprovado:** PROD possui apenas `staff_id` em SEP (migration `20260906000000`, sem trigger) enquanto STAGING tem `staff_id`+`professional_id`; `src/lib/finance/reversal.ts` lia exclusivamente `professional_id` → reversões em produção retornavam vazio. **Fix (branch `fix/reversal-sep-staff-id`, commits `bd0c8ce` fix + `d43eb21` harness):** `.select('*')` + contrato defensivo `staff_id || professional_id` nos 3 pontos (originais L185/L194/L220) — `SEP-IDENTITY-CONTRACT` NÃO assumido (frente futura; `professional_id` não declarado deprecado) + dual-write `staff_id`+`professional_id` no insert SEP do harness H2-8 (instrumento corrigido: baseline falhava 7.50 vs 3.75 porque `8ab6e14` passou o `participant-repository` a ler só `staff_id`, descartando o participante). **Validação:** typecheck/build OK (4 erros pré-existentes em script untracked), **45/45** testes-alvo, suíte completa **1509 PASS / 5 skip / 75 files**, harness H2-8 STAGING **pré-fix PASS e pós-fix PASS** (seed 3.75, originalCommission 3.75, netCommission 0, idempotente). **Gate 1:** PR **#85** → merge **`9b2aea8`** em `main`. **Gate 2 (config):** branch `test/h2-8-vitest-config`, commit único **`446a8f7`** = `vitest.h2-8.config.ts` (+28; `vite.config.ts` intocado — `tests/homologation/**` segue excluído do build e CI não executa o config) → CI obrigatória verde, `lint advisory` FAIL **aceito como baseline pelo PO** (33 errors/243 warnings pré-existentes, `continue-on-error`, fora do diff — **nenhuma correção autorizada**), pre-merge audit (1 commit/1 arquivo/base `9b2aea8`/0 tags), `/approve` → `review-bot-smg` APPROVED @ `446a8f7` → merge **`ea3d9f4`**. **Pós-merge:** diff `9b2aea8..ea3d9f4` = apenas `vitest.h2-8.config.ts`; zero migration/RPC/schema/RLS/ledger/CI; zero tag; `docs/adr/README.md` preservado (M pré-existente, nunca staged). **Deployment audit read-only (gate Produção):** integração Vercel automática (Git integration, **0 deploys manuais**) → 2× `Production – smg-barber` (`9b2aea8` 02:55, `ea3d9f4` 07:17) + previews staging/estetica + 3 previews na push da branch `446a8f7` — **todos `success`, 0 falhas, 0 pendências**; produção serve `main = ea3d9f4`; registrados 3 GAPs (DNS/domínio customizado, health HTTP, config interna Vercel) — *"deployment da plataforma validado read-only" ≠ "produção 100% validada"*. **Estado final:** DRIFT-FIX-REV-SEP-01 ✅ · H2-8 config ✅ · Deployment ✅ auditado; **próximo gate: B+C (H-8 topologia + smoke funcional de produção) — decisão do PO após este PR**; pendências separadas: delete branch `fix/reversal-sep-staff-id` 🟡, `SEP-IDENTITY-CONTRACT` 🟡, frente lint 🟡, D (P1-01) 🟡. **Executado:** branch `docs/registro-ciclo-drift-fix-reversal-h2-8`, ROADMAP 8.77, PROJECT_STATUS 7.38, `git diff --check` limpo, **docs-only (zero código/migration/SQL/RLS/RPC/CI/deploy)**. Registros: `PROJECT_STATUS.md` (7.38), PR #85, PR #86. |
| 2026-09-23 | 8.76 | **H-7 — P1-01 registrado + execução operacional ENCERRADA na janela 2026-09-23 (D-HOM-31): STOP OPERACIONAL, Q5–Q7 não executados.** Janela acompanhada (Rubens/UI produção + OpenCode read-only): baseline fresco capturado → **Q1–Q4 ✅ PASS** (comanda `b5368c28` R$145 cash, tx única, comissão HERON R$22,50) → fechamento profissional gravou **divergência falsa**: `barber_closing 728307e6` com **`status=discrepancy`/`expected_cash=0`/`cash_difference=145`** e `barber_closings_complete=true`. **Causa raiz em 3 níveis** da cadeia `payment_method` (transação `'cash'` → comanda `null` → loader `'Nao informado'` → lookup `['Dinheiro']`=0) + guarda de UI sem feedback — correção/validação **ponta-a-ponta** exigida antes de fechamento de caixa confiável. **Sem perda financeira; nenhuma correção aplicada; evidências BEFORE/AFTER preservadas.** **Decisão PO (D-HOM-31):** encerrar H-7 neste ponto, **Q5–Q7 não executados**, abrir frente de correção do P1; **`6bd5cbe4` preservada**. **STOP:** H-7 🔴; **H-8 🔴 BLOQUEADOR; sem merge, tag ou deploy**. Registros: `PROJECT_STATUS.md` (7.37), `docs/BUSINESS_DECISIONS.md` (D-HOM-31), `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md` (§10.7, §11). |
| 2026-09-23 | 8.75 | **H-3 — Chef Club 🟢 APROVADO (D-HOM-30) + H3-5 🟢 RESOLVIDO — ratificação formal do PO (2026-09-23).** Veredito do PO sobre a ressalva H3-5 (D-HOM-17/D-HOM-19) após **Gate C** (E2E `h35-managerroute-rbac.spec.ts`, tenant isolado `gatec-h35-*`, STAGING, **5/5 PASS**: bloqueio `barber`/`receptionist` nas 3 rotas via `ManagerRoute`, `manager`/`superadmin` acesso+conteúdo, `chef_club` ativa no plano `pro`; tenant real intocado) + validações unit **1399/5skip** · build · typecheck · `git diff --check` verdes. **H-3 deixa de possuir a ressalva H3-5 (H3-1..H3-6 ✅).** **Numeração:** identificador formal **D-HOM-30** — D-HOM-28/29 preservados como referências históricas órfãs (8.28 / H10 §6): *um identificador de decisão nunca deve ter dois significados* (PO). **Restrições (PO):** merge/tag/deploy **não autorizados por esta decisão** (gates separados); **H-8 segue 🔴**. Registros: `PROJECT_STATUS.md` (7.36), `docs/BUSINESS_DECISIONS.md` (D-HOM-30), `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (STATUS/§12), `docs/audit/H3_CHEF_CLUB_PERMISSIONS_VALIDATION.md` (§7), `docs/RELEASE_CHECKLIST_v1.5.md` ([x] H-3), PR #79. |
| 2026-09-23 | 8.74 | **H-3 — Ressalva H3-5 (permissões ManagerRoute) RESOLVIDA — Gate C E2E 5/5 PASS em tenant E2E isolado (2026-09-23, frente aberta pelo PO "H-3 com ressalva H3-5"; aguarda ratificação formal do PO para 🟢).** Exigência de D-HOM-17/D-HOM-19: exercitar o bloqueio de `barber`/`receptionist` fora do tenant real (Sanchez Barber só tem manager/superadmin). **Executado:** `tests/e2e/homologation/h35-managerroute-rbac.spec.ts` em STAGING (`tjcvuhynckocmvtqykxp`, `E2E_PROVISIONING=1`) com tenant isolado `gatec-h35-*` (`70aa026b-…`, provisionado + teardown e2e-seed; **tenant real intocado**): H35-1/2 barber/receptionist bloqueados nas 3 rotas Chef Club (ManagerRoute → `/#/dashboard` ×3) · H35-3 manager acesso+conteúdo · H35-4 owner/superadmin acesso+conteúdo · H35-5 feature `chef_club` ativa (`pro`) — **5/5 PASS (1.2m)**. **Validações da subfase:** unit **1399 PASS/5 skip** · build exit 0 · typecheck exit 0 · `git diff --check` limpo. **Registros:** `PROJECT_STATUS.md` (7.35), `docs/audit/H3_CHEF_CLUB_PERMISSIONS_VALIDATION.md` (§7), `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (STATUS/H3-5), `docs/RELEASE_CHECKLIST_v1.5.md` (H-3). **STOP:** H-3 permanece **🟡** até ratificação do PO (D-HOM a definir → 🟢); H-8 segue 🔴; **sem merge, tag ou deploy** (PR docs-only aguarda `/approve`). |
| 2026-09-18 | 8.73 | **P0.4 — Contas a Pagar + recorrentes 🟢 PROMOVIDA formalmente (2026-09-18, autorização do PO — frente P0.4; gate de promoção formal §18 do STOP GATE).** Promoção de **implementação previamente validada** (CONCLUÍDA 2026-09-11, ROADMAP 8.44 — Contas a Pagar com idempotência total A7/A8): 3 migrations (`20260905044639` accounts_payable, `20260905140000` A7 idempotency, `20260905150000` A8 recurring idempotency), RPCs `pay_account_payable`/`cancel_account_payable`/`create_accounts_payable_from_recurring`/`create_one_time_account_payable`/`create_recurring_bill`, **E2E STAGING 18/18 PASS** (runId `1789228868471`), aprovada no B2 fase 2 (ACL 5/5, objetos com RLS + idempotency, ledger 3/3). **Estado vivo revalidado antes da promoção** (read-only PROD `ushsnmlbeurfvlkieiln`, Management API, 2026-09-18): ACL **5/5** sãs (`{postgres, authenticated}`; anon/service_role=no), `accounts_payable`/`recurring_bills` `RLS=true` + `idempotency_key`, ledger **3/3** (`20260905044639`/`20260905140000`/`20260905150000`), drift `create_accounts_payable_from_recurring` **estável** (md5 `5c87853fc0ebfbb0408a59a07e1d13a0`/1053, idêntico à evidência bruta B2) — convergência `p04-convergence.mjs`: **ACL/LEDGER/DRIFT-MATCH true → CONVERGENT**. **Executado:** branch `docs/6.1.4-b2-gate-p0-4-promocao` (de `17516bb`); gate doc §0/§18; B2 fase 2 doc (P0.4 → PROMOVIDA); ROADMAP 8.73; PROJECT_STATUS 7.34; `git diff --check` limpo; **docs-only (zero código/migration/SQL/RLS/RPC)**; drift preservado (decisão PO D2). **STOP:** merge do PR **NÃO autorizado** — decisão separada do PO após revisão do diff; **deploy NÃO autorizado** (gate próprio, sequência: P0.4 ✅ → STOP → deploy → STOP). Registros: `PROJECT_STATUS.md` (7.34), `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§18). |
| 2026-09-14 | 8.71 | **F2.2 — SEC-AUTHZ-USAGE-MONITOR (edge function pública sem autenticação com `service_role`) 🟢 CLOSED — merged (2026-09-14, ciclo smg-change-control completo, SEM deploy — a função permanece não deployada).** Achado da auditoria de segurança (Categoria 2/média, ISSUE 6): `supabase-usage-monitor` criava client com `service_role` (l.42-48) e aceitava POST **sem validar `Authorization`**, gravando em `usage_logs`/`alerts` com bypass de RLS (policies `is_super_admin()` preservadas). **Fix (branch `security/f2.2-usage-monitor-auth`, commit `acf6d77`):** cadeia completa `request → autenticação → autorização → validação do payload → operação service_role` — novo módulo puro `supabase/functions/_shared/usage-monitor-auth.ts` (`isSuperAdminUser` espelha `public.is_super_admin()`; `validateMetricsPayload` com whitelist dos 6 resource types canônicos do dashboard, números finitos/positivos, `MAX_METRICS_PER_BATCH=100`, unidade obrigatória); `index.ts` reescrito POST-only (GET/mock removidos → 405), `auth.getUser(token)` via GoTrue (anon key), 403 para não-superadmin, 400 para payload inválido, client `service_role` criado **somente após** 1-3; `config.toml` novo (`verify_jwt = false` explícito + comentário de governança — auth vive na função, não no gateway, padrão F2.1/worker-dispatcher). **Testes:** `tests/security/f2_2_usage_monitor_auth.test.ts` — **20/20 PASS**; suíte `tests/security` **77/77 PASS** (zero regressão F2.1/F3.1/P1.3/F5.1-2); `npm run typecheck` exit 0; `git diff --check` limpo. **Gate formal (smg-change-control):** commit `acf6d77` → branch pushada (autorização PO) → **PR #61** → CI verde (`architecture:ci`/`build`/`typecheck`/`unit`/`validate` + Vercel ×3 PASS; `lint advisory` = baseline pré-existente) → **`/approve` do PO → `review-bot-smg` APPROVED** → **merge squash em `main` = `c42bdbb2` (#61)**. **SEM deploy:** nenhuma edge function deployada em PROD/STAGING (função permanece não deployada até validação em ambiente controlado); RLS/grants das tabelas `usage_logs`/`alerts`/`notification_channels` **intocados**; **`import-export` com `verify_jwt=False` em PROD = achado separado, própria frente (não absorvido)**. **Registros:** `PROJECT_STATUS.md` (7.32). |
| 2026-09-14 | 8.70 | **F3.1 — `approve_access_request` SEM AUTORIZAÇÃO 🟢 CLOSED — aplicada e validada em PRODUÇÃO (2026-09-14, ciclo smg-change-control completo + gates STAGING/PROD do PO).** Achado da auditoria de segurança (Categoria 2/Alta; dívida P3 D-HOM-26): RPC legada `approve_access_request` **sem check `auth.uid()`/superadmin** — qualquer usuário autenticado podia aprovar pedidos de acesso. **Fix** (branches `security/f3.1-approve-access-request` e `security/f3.1a-approve-access-request`, merges **`0bf8d4f` (#58)** e **`32761db` (#59)** em `main`): migration `20260914120000_fix_approve_access_request_auth_guard.sql` com `IF auth.uid() IS NULL THEN RAISE EXCEPTION ... END IF;` (guarda de autenticação) + `IF NOT current_is_super_admin_from_auth_uid() THEN RAISE EXCEPTION ... END IF;` (guarda de superadmin, via helper SECURITY DEFINER existente) + `SET search_path TO 'public', 'auth'` (emenda F3.1a, hardenning de ACL), preservando `SECURITY DEFINER` e o corpo de negócio (grants de `authenticated` mantidos — sem `anon`/`PUBLIC`). **Validação:** vitest **1373 PASS / 5 skipped** (suíte completa) + typecheck limpo + build OK + `git diff --check` limpo. **STAGING** (`tjcvuhynckocmvtqykxp`): aplicada via Management API (HTTP 200; `name=20260914120000`, version efetiva `20260914215237`); verificado: guardas presentes, `search_path` presente, `SECURITY DEFINER`, proacl `{postgres, authenticated, service_role}` **sem `anon`**; **E2E H6-11 PASS** (probe fiel: manager comum → `P0001 Insufficient permissions: superadmin required to approve access requests`, pedido `pending`, teardown limpo). **PROD** (`ushsnmlbeurfvlkieiln`): aplicada via Management API (**HTTP 200**; somente `20260914120000`, M4/P2.1 **intocadas**; version efetiva **`20260914224139`**); verificado: `has_auth_guard=true`, `has_superadmin_guard=true`, `has_search_path=true`, `prosecdef=true`, proacl `{postgres, authenticated, service_role}` **sem `anon`**; **E2E H6-11 PASS em PROD**: manager comum → `P0001 Insufficient permissions: superadmin required to approve access requests`, pedido permanece **`pending`**, teardown completo. **Higiene:** `.env.local` preservado (hash conferido), worktree limpo, config temporária de E2E removida. **Frente separada (NÃO tocada):** suíte `h6-security.spec.ts` stale (fixtures `products.price` vs `cost_price/sale_price`; `staff.role:'Manager'` vs constraint lowercase) — registrada como **achado colateral/frente prória**, aguarda próprio INTAKE/AUDIT/CLASSIFY. Registros: `PROJECT_STATUS.md` (7.31), `docs/security/SECURITY_AUDIT_RPC.md` (dívida P3 → RESOLVIDA). |
| 2026-09-14 | 8.69 | **F5.1+F5.2 — XSS nos fluxos de impressão (`document.write`) 🟢 CLOSED — merged (2026-09-14, ciclo smg-change-control completo, sem deploy — escopo front-end).** Achados da auditoria de segurança (Categoria 5 — Inputs/XSS, severidade alta): `handlePrint` de **Comandas** (F5.2) e **CashClosingPage** (F5.1) injetavam valores controlados pelo usuário em HTML via `document.write` sem sanitização — vetores: `product_name`, `clients.name`, `staff_names` (F5.2) e `previewText`/`formattedFilterDate` (F5.1), + vetor `about:blank + opener` escalável. **Fix (branch `security/f5.1-f5.2-xss-print`, commit `3d7b6c4`):** novo helper `shared/strings/escapeHtml.ts` (escape por entidades HTML `& < > " '`, sem dependência externa — recomendação P1 do relatório, DOMPurify não adicionado) aplicado a **todos** os valores interpolados dos dois `handlePrint`, + `printWindow.opener = null` em ambos (defesa em profundidade contra o vetor opener). **Testes:** `tests/security/f5_1_f5_2_xss_print_escape.test.ts` — **11/11 PASS** (script tag, `img onerror`, breakout de atributo `"`/`'`, ampersand duplo, payload misto de recibo). **Validação:** vitest **1366/1366** · typecheck limpo · build OK · `git diff --check` limpo. **Gate formal (smg-change-control):** commit `3d7b6c4` → branch pushada (autorização PO) → **PR #56** → CI verde (`typecheck/build/unit/architecture:ci/validate` PASS — obrigatório pela branch protection; `lint advisory` = baseline pré-existente, zero ocorrências nos 5 arquivos da frente) → **`/approve` do PO → `review-bot-smg` APPROVED** → **merge squash em `main` = `d3b279d` (#56)**. **Sem deploy:** front sem edge functions/migrations (deploy N/A). **Registros:** `PROJECT_STATUS.md` (7.30). |
| 2026-09-14 | 8.68 | **F4.1 — SECRETS (.vercel-temp.env commitado) 🟢 CLOSED — merged (2026-09-14, ciclo smg-change-control completo, sem deploy — escopo repo-only).** Achado da auditoria de segurança (Categoria 2): arquivo `.vercel-temp.env` foi commitado no repositório por engano, expondo variáveis sensíveis no histórico git. **Fix (branch `security/f4.1-git-secrets`, commit `124a099`):** `.vercel-temp.env` **removido do índice git** + nova regra de proteção no `.gitignore` (evita re-commit acidental). **Decisão do PO (2026-09-13):** commit + **skip** da rotação/filter-repo (nenhuma secret com risco real publicada em histórico). **Validação:** CI verde no PR (typecheck/build/unit/architecture:ci/validate PASS; `lint advisory` = baseline pré-existente). **Gate formal (smg-change-control):** sync com `origin/main` (merge `6829ece`, diff limpo = `.gitignore` +3 / `.vercel-temp.env` −41) → push autorizado pelo PO → **PR #54** → CI verde → **`/approve` do PO → `review-bot-smg` APPROVED** → **merge squash em `main` = `c1711da` (#54)**. Branch remota `security/f4.1-git-secrets` deletada pós-merge. **Sem deploy:** mudança repo-only (nenhuma edge function/migration afetada). **Registros:** `PROJECT_STATUS.md` (7.29). |
| 2026-09-13 | 8.67 | **F2.1 — SEC-AUTHZ-CREATE-USER (Escalação de privilégio: manager criava usuário admin via `admin-create-user`) 🟢 CLOSED — merged + deployado em PRODUÇÃO (2026-09-13, ciclo smg-change-control completo).** Achado da auditoria de segurança (Categoria 2/Alta): edge function `admin-create-user` aceitava `role` do body e a guarda antiga (`isManagerLike`) bloqueava APENAS `superadmin` — manager podia criar/promover `admin`. **Fix (branch `security/f2.1-authz-create-user`, commit `60586c4`):** hierarquia canônica em `supabase/functions/_shared/staff-role-hierarchy.ts` (rank superadmin=4, admin=3, manager/owner/gerente administrativo=2, barber/receptionist=1; regra `rank(requested) < rank(caller)`); guarda 403 na edge function (substitui código morto — `normalizeStaffRole` nunca retornava 'superadmin'); frontend `pages/Team.tsx` filtra `admin` do seletor para não-superadmins (`isSuperAdmin` + `roleOptions`, preserva papel atual na edição). **Testes:** `tests/security/f2_1_role_hierarchy.test.ts` — **25/25 PASS** (matriz de aceite + borders). **Validação:** vitest 25/25 · typecheck limpo · build OK · `git diff --check` limpo. **Gate formal (smg-change-control):** commit `60586c4` → branch pushada → **PR #52** (CI verde: typecheck/build/unit/architecture:ci/validate PASS; `lint advisory` = baseline pré-existente) → **`/approve` do PO → revisão formal `review-bot-smg` APPROVED** (mecanismo `smg-pr-approve`, idempotente; re-aprovação pós-merge de `origin/main`) → **merge squash em `main` = `99cf820` (#52)** → **deploy autorizado pelo PO** → `supabase functions deploy admin-create-user --project-ref ushsnmlbeurfvlkieiln --use-api` → **v24 ACTIVE, `verify_jwt=true` preservado** (entrypoint `.../admin-create-user/index.ts`, dependência `_shared/staff-role-hierarchy.ts` bundlada). **POST-DEPLOY verificado:** `supabase functions list` → `admin-create-user` ACTIVE, version 24, `verify_jwt: true`, entrypoint correto. **GAP registrado (F1):** vetor de edição direta `staffRepository.update` (RLS) não passa pela checagem de hierarquia — enforcement backend de edição de papel pertence à F1; efeitos laterais aceitos (PO): manager→manager agora bloqueado (leitura estrita da correção) e staff `admin` (não-superadmin) também não atribui `admin`. **Registros:** `PROJECT_STATUS.md` (7.28), `docs/DOCUMENTACAO_COMPLETA_SISTEMA.md` (§7.7). F4.1 (SECRETS) permanece commitada com PUSH pendente em `security/f4.1-git-secrets` (`124a099`). |
| 2026-09-12 | 8.66 | **P0.3-C — Cancelar Cobrança ≠ Dar Baixa 🟢 PROMOVIDA formalmente (2026-09-12, decisão formal do PO — promoção documental).** *"P0.3-C → promoção formal. Revalidação read-only já convergiu. PROD está conforme o baseline. STAGING tem 9/9 E2E. Código já está em main. Portanto, não há justificativa para uma nova implementação funcional. reverse_revenue → manter fora. O OpenCode deve trabalhar somente na formalização documental da promoção: Change Control; isolamento; atualização dos documentos necessários; validação; commit; push; PR; reviewer independente; STOP antes do merge. No PR, deixar absolutamente explícito: P0.3-C — promoção de implementação previamente validada. Nenhuma nova implementação funcional. Nenhuma alteração de RPC, migration, RLS, ACL, EventStore ou regra financeira. reverse_revenue permanece desativado. Depois do PR → STOP."* **Base do gate:** P0.3-C **CONCLUÍDA** (ROADMAP 8.54, 2026-09-11) — código em `main` (proveniência `84bab79` P0.3 + `6d96996` E2E seed); D3 saneamento ACL das 2 RPCs em PROD; **aprovada no gate read-only B2 fase 2** (8.61: ACL 2/2 sãs, objeto `receivable_cancel_audit` presente com RLS, ledger 4/4); **E2E STAGING 9/9** (runId `1789104229885`). **Revalidação do estado vivo antes da promoção (read-only PROD `ushsnmlbeurfvlkieiln`, 2026-09-12):** ACL **2/2** P0.3 sãs (`cancel_club_receivables`, `cancel_subscription_with_receivables` — SECURITY DEFINER, proacl `{postgres, authenticated}`, `anon=no`/`service_role=no`); `receivable_cancel_audit` com `RLS=true` + policy `receivable_cancel_audit_tenant_isolation` + 4 índices; ledger **4/4** (`20260903000000`–`0003`); **hash SHA-256 idêntico ao B2 fase 2 — ACL True, ledger True** (`p03-convergence.mjs`; o relatório final P0.3 de 11/09 registrava `0001–0003` ausentes — D4 🔴, corrigido pelo repair B3/ROADMAP 8.59 e confirmado 4/4 na validação B2 fase 2). **Executado:** branch `docs/6.1.4-b2-gate-p0-3-promocao` isolada de `9553af7`; gate doc atualizado (§0 P0.3 → PROMOVIDO + nova §17 PROMOÇÃO FORMAL); B2 fase 2 doc atualizado (P0.3 → PROMOVIDA); ROADMAP 8.66 + PROJECT_STATUS 7.27. **Natureza:** fechamento documental — **zero alteração de código/banco/migrations/RLS/RPC/regra financeira**; `reverse_revenue` permanece **desativado** (matriz FinanceSubscriber `SubscriptionCancelled → NONE`, ADR-001 intacta). **Validação:** `git diff --check` limpo; diff docs-only (zero migration/SQL/TS). **STOP:** merge do PR **NÃO autorizado** — decisão separada do PO após revisão do diff (*"Depois do PR → STOP"*); **P0.4 e deploy NÃO autorizados neste ato** (sequência do PO: P2.1 ✅ → STOP → M4/E2E ✅ → STOP → P0.3 ✅ → STOP → P0.4/deploy → STOP). Registros: `PROJECT_STATUS.md` (7.27), `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§17), `docs/audit/6.1.4_B2_FASE2_VALIDACAO_FRENTES_A.md` (P0.3). |
| 2026-09-12 | 8.65 | **M4 — E2E SUITE BASELINE EXECUTADA 🟢 CICLO M4/E2E ENCERRADO (2026-09-12, autorização do PO item "2" — M4 como próxima frente da sequência).** Pendência do ROADMAP 8.53 / B2 fase 2 executada: **E2E suite baseline da frente M4**. **Revalidação do estado vivo PROD `ushsnmlbeurfvlkieiln` antes da execução (read-only, 2026-09-12):** ACL **8/8** funções M4 sãs (`SECURITY DEFINER`, proacl `{postgres, authenticated}`, `anon=no`/`service_role=no`); ledger **14/14** no range (5 migrations M4 `20260830000000`–`20260830040000` + saneamento `20260911120000` + demais frentes); **hash SHA-256 idêntico ao B2 fase 2 — ACL True, ledger True** (estado vivo inalterado). **E2E STAGING `tjcvuhynckocmvtqykxp`:** suíte `tests/e2e/smoke/core.spec.ts` **10/10 PASS** (runId `1789228868471`, ~1.2 min, `--workers=1` padrão CI, teardown completo — tenant `e2e-suite-1789228868471` criado/removido). Observação de método: 1ª execução local com workers paralelos = 4/10 por **cold-compile do Vite** (`Carregando módulo...`, padrão documentado em `playwright.config.ts`); reexecução padrão CI → 10/10 **sem nenhuma alteração de código/testes** (não é regressão). **STOP:** ciclo M4/E2E encerrado; **P0.3 NÃO autorizado neste ato** (sequência do PO: P2.1 ✅ → STOP → M4/E2E ✅ → STOP → P0.3 → STOP → P0.4/deploy → STOP); zero alteração de código/banco/migrations/RLS/RPC/regra financeira; merge/deploy não executados. Registros: `PROJECT_STATUS.md` (7.26), `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§16), `docs/audit/6.1.4_B2_FASE2_VALIDACAO_FRENTES_A.md` (M4). |
| 2026-09-12 | 8.64 | **P2.1 — Import Engine + CSV de Clientes ✅ GATE FORMALMENTE CONCLUÍDO PÓS-MERGE (2026-09-12).** Registro do fato consumado: **merge do PR #48 executado** — reviewer independente `review-bot-smg` **APPROVED** → merge em `main` como `d9d9ef4800991ac542ea6627668fbe0335925e1e` (2026-09-12T15:34:25Z, "Merge pull request #48 from augustosanchesmanager-ux/gate/6.1.4-promocao-p2-1"). PR **docs-only** (4 arquivos, +62/−9 — ROADMAP 8.63, PROJECT_STATUS 7.24, gate doc §15, B2 fase 2 doc); zero código/SQL/migration. **Validações pós-merge:** `validate`/`build`/`typecheck`/`unit`/`architecture:ci`/Vercel Preview PASS; `lint advisory` = baseline pré-existente (advisory). **Ancestralidade:** `9a86b2d` presente em `main` (exit 0). **Evidências B2 mantidas válidas** (objeto PROD não tocado). **Autorização do merge e próxima frente:** PO 2026-09-12 ("1 e 2") — (1) merge autorizado e executado; (2) **M4 autorizado como próxima frente** (sequência: M4/E2E → STOP → P0.3 → STOP → P0.4/deploy → STOP). Registros: `PROJECT_STATUS.md` (7.25), `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§15.6). |
| 2026-09-12 | 8.63 | **P2.1 — Import Engine + CSV de Clientes 🟢 PROMOVIDA oficialmente (2026-09-12, autorização formal do PO).** *"Autorizar somente a promoção formal da P2.1. Não autorizar M4, P0.3 ou P0.4 neste ato. Não autorizar alterações de código, banco, migrations, RLS, RPC ou regra financeira. Executar o ciclo governado completo aplicável, validar o estado vivo antes da alteração e parar no STOP GATE antes de qualquer merge/deploy não explicitamente autorizado."* **Sequência do PO (verbatim):** *"P2.1 → STOP → M4/E2E → STOP → P0.3 → STOP → P0.4/deploy → STOP — não porque a ordem seja tecnicamente obrigatória... mas porque reduz o blast radius e mantém uma frente por vez sob controle do PO."* **Base do gate:** P2.1 já **PRODUCTION CERTIFIED** (8.49, 2026-09-08) — migration `20260906130000_p2_1_import_persistence.sql` (VERSÃO FINAL CERTIFICADA, D5/8.50) aplicada e validada em PROD; código em `main` (PR #27 `66922f4`, `feature/p2-1-csv-clientes`: `pages/Clients.tsx`, `src/modules/import-engine/*`, `domain/client/*`, E2E `p2-1-client-import.spec.ts`); aprovada no gate read-only (B2 fase 2, 8.61: ACL 1/1, objetos com RLS, ledger 1/1). **Revalidação do estado vivo antes da promoção (read-only PROD `ushsnmlbeurfvlkieiln`, 2026-09-12):** `import_clients_batch(uuid,jsonb)` SECURITY DEFINER com proacl `{postgres, authenticated}` (`anon=no`/`service_role=no`/`authenticated=YES`); `import_jobs`/`import_rows` `RLS=true`; ledger `20260906130000` = `p2_1_import_persistence` registrada; policies `tenant_isolation_*` + `superadmin_bypass_*` presentes. **Executado:** branch `gate/6.1.4-promocao-p2-1` isolada de `origin/main` (3bdf747); gate doc atualizado (§0 P2.1 → PROMOVIDO + nova §15 PROMOÇÃO FORMAL); B2 fase 2 doc atualizado (P2.1 → PROMOVIDA); ROADMAP 8.63 + PROJECT_STATUS 7.24. **Natureza:** fechamento documental — **zero alteração de código/banco/migrations/RLS/RPC/regra financeira**. **Validação:** `git diff --check` limpo; diff docs-only (zero migration/SQL/TS). **STOP:** merge do PR e deploy **NÃO executados** — aguardam autorização explícita do PO (branch protection de `main`: 1 approving review de reviewer independente, sem contorno — PO-6.1.4-B2-03). **M4, P0.3 e P0.4 NÃO autorizados neste ato** — seguem candidatas na sequência do PO. Registros: `PROJECT_STATUS.md` (7.24), `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§15). |
| 2026-09-12 | 8.62 | **6.1.4-B — P1.3 → P1.1 FORMALMENTE CONCLUÍDO PÓS-MERGE 🟢 (2026-09-12, autorização explícita do PO para formalização documental).** Registro do encerramento formal do gate de promoção: o **merge do PR #31 já foi executado** — aprovação do reviewer independente `review-bot-smg` (**APPROVED 2026-09-10T22:49:48Z**) → **merge em `main` como `7e2f2cf3fba46cf5e5f492396c95b07e4e289ec5`** (2026-09-10T22:51:20Z, "Merge pull request #31 from augustosanchesmanager-ux/gate/6.1.4-promocao-p1-3-p1-1"). **Natureza:** docs-only — 6 arquivos, +669 (PROJECT_STATUS, ROADMAP, 3 docs de auditoria 6.1.4 + script de rollback B1 como material documental); zero código de aplicação/migration nova — consolidação documental do gate (entradas 8.51/8.52). **Validações pós-merge verdes:** `validate`/`build`/`typecheck`/`unit`/`architecture:ci`/Vercel Preview PASS; `lint advisory` = baseline pré-existente (8.50); `git merge-base --is-ancestor 7e2f2cf origin/main` → exit 0. **Gap documental corrigido:** ROADMAP 8.52 / PROJECT_STATUS 7.13 / gate doc §13.2/§13.4/§14.1/§14.2 apresentavam o merge como "aguardando reviewer independente" (estado stale pré-merge) — atualizados para refletir o fato consumado, preservando a sequência histórica (STOP → APPROVED → MERGED → pós-merge validado → gate concluído). **STOP:** nenhuma ação produtiva/deploy/merge adicional; frente NÃO reaberta (P1.3/P1.1 já CLOSED — 8.48/8.47). Registros: `PROJECT_STATUS.md` (7.23), `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§13.5). |
| 2026-09-12 | 8.61 | **6.1.4-B2 FASE 2 — VALIDAÇÃO FORMAL DAS 4 FRENTES A 🟡 VEREDITO APRESENTADO (2026-09-12, autorização do PO via seleção "B2 fase 2 — validação formal das 4 frentes A").** Gate **read-only** (Management API, ref PROD `ushsnmlbeurfvlkieiln`, 8 queries q1–q8 — zero escrita): **4/4 frentes A APROVADAS** — (1) **ACL 16/16 funções sãs** (`SECURITY DEFINER`, proacl `{postgres, authenticated}`, `anon=no`/`service_role=no` em todas); (2) **objetos presentes com RLS** (`accounts_payable`+`recurring_bills` com `idempotency_key`, `import_jobs`/`import_rows`, `appointment_attendance_corrections`, `receivable_cancel_audit` + 3 índices); (3) **ledger PROD 14/15 registrado** (M4 5/5 + saneamento `20260911120000`, P0.3 4/4, P0.4 3/3, P2.1 1/1; ausente apenas `20260912120000` com efeito confirmado presente — REVOKE executado pela frente ACL fora do runner); (4) **drift P0.4 reexecutado estável** (`5c87853c…`/1053 — idêntico ao catalogado, corpo preservado conforme decisão do PO); (5) **não-dependência dos itens D confirmada** (matriz A.1 §6: tabelas piloto das frentes A idênticas STG×PROD; escritas via RPC SECURITY DEFINER, leituras em massa com `.eq('tenant_id')`, leituras por id via policies próprias — nenhuma das 13 tabelas D envolvida). Observações não-bloqueantes: `control.import_jobs` (artefato de outra frente, vazio); ausência documental de `20260912120000` no ledger. **STOP:** promoção formal das frentes + destravamento (P0.4 deploy/homologação, M4 E2E suite, encerramento 6.1.4-B) exigem decisão explícita do PO. Registros: `PROJECT_STATUS.md` (7.22), `docs/audit/6.1.4_B2_FASE2_VALIDACAO_FRENTES_A.md` (evidências §2/§3/§4, STOP §7). |
| 2026-09-12 | 8.60 | **Frente ACL `20260908120000` (incidente P2.1-OUTBOX-42501) 🟢 ENCERRADA — BASELINE CERTIFICADA `v2.1.0-acl-20260908120000-certified` (2026-09-12, PO-ACL-11/12 autorizados pelo PO).** **Merge:** PR #44 squash → `main@a006ec43af83069d4e73a15648a683dc594b5156` (2026-09-12T06:21:45Z, mergedBy PO; CI pós-merge `validate`/`test`/`build`/`unit`/`typecheck`/`architecture:ci` success — runs 34677927811/34677927845; `lint advisory` = baseline pré-existente). **Deploy PO-ACL-10:** produção Vercel confirmada servindo `a006ec4` — deployment `Production – smg-barber` success (sha `a006ec4`, 2026-09-12T06:22:22Z) + vendor chunk byte-idêntico ao build local + session guard presente no bundle ativo `index-CFn0quXb.js`. **Smoke pós-deploy (Playwright, `barber.soumanager.com`):** anônimo zero 42501/zero queries outbox sem sessão (guard ativo); autenticado funcional (login homologação D-HOM-11 OK, dashboard acessado, dispatcher opera com sessão — 10 queries outbox, 0 erros). Observação não-bloqueante: 1× PGRST116 (406) em `tenant_settings` — `.single()` com 0 linhas, pré-existente, fora do escopo. **Baseline:** tag anotada `v2.1.0-acl-20260908120000-certified` → `a006ec4` (push OK). **Registros:** `PROJECT_STATUS.md` (7.21), `docs/audit/6.1.4_ACL_20260908120000_ABERTURA.md` (§10.11 merge, §10.12 deploy, §10.13 baseline, §11 STOP). |
| 2026-09-11 | 8.59 | **6.1.4-B3 — GAP DE PROVENIÊNCIA DO LEDGER PROD 🟢 FECHADO (reconciliação das 7 versões, autorização explícita do PO 2026-09-11).** Investigação read-only autorizada (PO-6.1.4-B3-01/02) → proveniência das séries P0.3/P0.4/P2.1 comprovada objeto a objeto em PROD (`ushsnmlbeurfvlkieiln`): todas as 7 funções/tabelas presentes com ACL saneada `{postgres, authenticated}` (SECURITY DEFINER, sem anon/service_role) e bodies idênticos ao repo. **STOP GATE apresentado → decisões do PO:** (1) repair das 7 versões AUTORIZADO; (2) `20260906000000` (classe C) **mantido ausente** do ledger — representação correta (no-op confirmado em PROD: `staff_id` presente, `professional_id` ausente); (3) `20260908120000` (grant anon nas 2 RLS helpers) **fora da reconciliação** — destino funcional/security do grant será **frente futura separada** (nem repair, nem REVOKE agora). **Repair executado:** `supabase migration repair --status applied` das 7 versões (`20260903000001/2/3`, `20260905044639`, `20260905140000`, `20260905150000`, `20260906130000`) via CLI (env `SUPABASE_DISABLE_UPDATE_CHECK=true`; saída JSON `{"versions":[...],"status":"applied","message":"Migration history repaired"}`) — registro de metadado, sem reexecução de SQL. **Verificação pós-repair:** 7/7 versões presentes com `name` correto; ledger 142 (pré) + 7 = **149 (pós)** via `count(*)` + `NOT IN`; 8 RPCs das séries intactos. **Encerramento do gap:** proveniência comprovada + ledger reconciliado + evidências documentadas (método `curl.exe` Management API, `Invoke-RestMethod` descartado por travamento). Registros: `PROJECT_STATUS.md` (7.20), `docs/audit/6.1.4_B3_LEDGER_PROVENIENCIA.md` (§11 evidências, §12 registro pós-repair). Frente futura separada: decisão funcional/security do grant anon de `20260908120000`. |
| 2026-09-11 | 8.58 | **SMG — Automação de Aprovação Controlada de PR 🟢 FECHADA — IDEMPOTÊNCIA G7 VALIDADA EM PRODUÇÃO (2026-09-11, PRs #39 `49dde4a` + #40 `c357b3f` mergeados, autorização explícita do PO em ambos).** Prova real final no PR #40 (duplo `/approve` do PO com o código corrigido já em `main`): 1º run `34654753835` success → `APPROVED` (`review-bot-smg`, id `5183968634`, head `c3e080e`); 2º run `34654830824` success com log **`NOOP - PR já possui aprovação registrada (idempotência - nada a fazer)`** → **exatamente 1 review**, zero duplicata (G7 exit 0 sem POST — comportamento corrigido em `guards.mjs`/`run.mjs`, 23/23 vitest). **Ciclo validado de ponta a ponta:** happy path (3× PR #39 + 1× PR #40), `dismiss_stale_reviews` (re-aprovação pós-commit), idempotência com código corrigido, CI pós-merge em `main` verde (`validate`/`test`/`build`/`unit`/`typecheck`/`architecture:ci` ✅; `lint advisory` = baseline 8.50). Bootstrap confirmado em todos os runs (código de `main` no `issue_comment`, nunca da branch do PR). **Front chaveada para operação:** `/approve` do PO segue registrando APPROVED como `review-bot-smg`; merge continua exigindo autorização explícita do PO; auto-merge permanece proibido (PO verbatim). Registros: `PROJECT_STATUS.md` (7.19), `docs/audit/6.1.4_SMG_PR_APPROVE_AUTOMATION.md` §6.1. |
| 2026-09-11 | 8.57 | **SMG — Automação de Aprovação Controlada de PR 🟢 OPERACIONAL + VALIDADA POR PROVA REAL (2026-09-11, PR #39 squash merged, SHA `49dde4a`).** **Validação operacional em PR controlado #39** (`docs/pr-approve-merge-record`): 3 runs `/approve` com sucesso real (runs `34652689814`, `34653049232`, `34653455453`) → review `APPROVED` emitido por `review-bot-smg` em cada head (22:09:06 / 22:13:48 / 22:19:15Z) — ciclo completo validado: trigger `issue_comment` em PR → gates G1–G6 PASS → POST `/pulls/{n}/reviews {event: APPROVE}` → branch protection satisfeita (1 approving review, conta não-autora). **Bootstrap confirmado:** workflow executa a partir de `main` (`actions/checkout@v4` sem `ref` em evento `issue_comment` → código de `main`, nunca da branch do PR — desenho correto: a aprovação jamais executa código do PR sob revisão). **Bug capturado pela prova real e corrigido:** idempotência G7 retornava exit 1 (fail), contratado como **STOP silencioso exit 0 sem POST** — corrigido em `guards.mjs`/`run.mjs` (resultado `noop`, novo teste dedicado; **23/23 vitest PASS**); fix entregue no PR #39 e mergeado (`49dde4a`). **Descoberta de credencial:** fine-grained PAT (especificado) falhou 3× com `403 Resource not accessible by personal access token` no POST de reviews — causa: permissão de recurso `Pull requests: Write` ausente na configuração GIF; PO adotou **token clássico com scope `repo`** da `review-bot-smg` (mesmo tipo que aprovou PRs #35/#37 via CLI) — **1ª tentativa pós-troca: sucesso**; trade-off de least-privilege registrado para migração futura. **Descoberta de arquitetura:** `dismiss_stale_reviews: true` no branch protection → novo commit invalida aprovação (dismiss automático do review) e exige novo `/approve` no head atual — comportamento observado e documentado (ciclo completo revalidado). **Governança:** §6.1 atualizada com evidências completas em `docs/audit/6.1.4_SMG_PR_APPROVE_AUTOMATION.md`. Testes pós-merge em main: **`validate`/`test`/`build`/`unit`/`typecheck`/`architecture:ci` todos success** (`lint advisory` = baseline ROADMAP 8.50). **Auto-merge continua proibido** (PO verbatim) — `/approve` desbloqueia a proteção; merge exige autorização explícita do PO em cada ocorrência. Registros: `PROJECT_STATUS.md` (7.18). |
| 2026-09-11 | 8.56 | **SMG — Automação de Aprovação Controlada de PR 🟢 MERGED ✅ (2026-09-11, PR #37, SHA d16bd48, squash merge autorizado pelo PO).** Workflow `.github/workflows/smg-approve.yml` (`smg-pr-approve`, trigger `issue_comment[created]` com pré-filtro `body == '/approve'` + `issue.pull_request` + `permissions: contents: read`): PO comenta `/approve` em um PR → `scripts/smg-pr-approve/run.mjs` (Node 22 nativo, zero dependência nova) aplica **gates fail-closed** (`guards.mjs`): G0 comentário em PR → G1 autor == PO `augustosanchesmanager-ux` → G2 comando exato → G3 PR aberto → G4 não-draft → G5 base `main` → G6 CI obrigatório todo `completed`+verde (exceção allowlist `lint advisory` = baseline ROADMAP 8.50) → G7 sem `APPROVED` existente → **somente** se TODOS PASS, POST `/pulls/{n}/reviews {event: APPROVE}` como conta `review-bot-smg` via PAT fine-grained `REVIEW_BOT_TOKEN` (secret do repo, escopo `Pull requests: Write` apenas, criado diretamente pelo PO — nunca compartilhado com o agente). **Fora do escopo (proibido, PO verbatim):** auto-merge ("Eu não colocaria o merge automático nessa primeira versão"), deploy, alteração de branch protection. Terminologia oficial: `review-bot-smg` = **"review executor automatizado"** (executa a decisão do PO; NÃO é "reviewer independente"). Qualquer gate FAIL → exit 1 (STOP, job vermelho, nenhuma aprovação). Idempotência: APPROVED existente → STOP sem POST. Testes: `scripts/smg-pr-approve/guards.test.mjs` — **22/22 vitest PASS** (todos os caminhos fail-closed: comando/autoridade/PR/draft/base/CI/idempotência/composição). Validação local: `node --check` run.mjs+guards.mjs OK · `git diff --check` OK. Governança completa (escopo/travas/segredos/plano de teste em PR controlado — nunca #35/#36): `docs/audit/6.1.4_SMG_PR_APPROVE_AUTOMATION.md`. Branch: `feat/smg-pr-approve-automation`. **MERGE EXECUTADO:** PR #37 squash merged (SHA `d16bd48`, 2026-09-11T19:41:40Z, authorize pelo PO via `review-bot-smg` APPROVED). **Bootstrap concluído:** workflow `smg-approve.yml` agora presente em `main` — automação operacional em PRs futuros. PR controlado de validação em andamento. Registros: `PROJECT_STATUS.md` (7.17). |
| 2026-09-11 | 8.55 | **P0.4 — Contas a Pagar + Contas Recorrentes: ABERTURA/PREFLIGHT CONCLUÍDA (STOP GATE RESPONDIDO — D1–D7, PO 2026-09-11).** Gate: `docs/audit/6.1.4_P0_4_STOP_GATE.md` (§5 verbatim). **Drift B3 catalogado (D2 ✅):** `create_accounts_payable_from_recurring` em PROD — corpo **ASCII-normalizado** (len 1053, md5 `5c87853c…`, sem acentos/comentários) vs repo/STAGING (len 1282, md5 `7292cf8b…`, acentos + comentários); SQL funcionalmente equivalente; evidência em `docs/audit/6.1.4_P0_4_DRIFT_EVIDENCIA_B3.md`; **sem alteração de corpo** (overwrite/DROP/CREATE/repair proibidos). **Proveniência:** migration `20260905044639` criada em `e9761e5` (2026-09-05), nunca modificada; worktree limpo; STAGING == repo byte-for-byte; PROD aplicado fora do migration runner (ledger 0/3 — **D4 ❌ NÃO registrar/repairar**). **D5 ACL ✅:** 5/5 funções P0.4 sane e idênticas PROD=STAGING (`security_definer=true`, `anon_exec=false`, `authenticated_exec=true`, `service_role_exec=false`, proacl `{postgres, authenticated}`). **D7 ✅:** vitest `domain/accountsPayable/domain.test.ts` **13/13 PASS**; `npm run typecheck` limpo; `npm run build` limpo. **D3 ✅:** E2E `flow-p0-4-accounts-payable.spec.ts` em STAGING **18/18 PASS** (runId `1789110611512`, tenant `e2e-suite-1789110611512`; teardown: tenant órfão deixado p/ operador — padrão conhecido). **D6 ✅ MERGE EXECUTADO (PR #35 → `main`, merge `fc55d32`, 2026-09-11T15:38:08Z):** cadeia do PO cumprida — *"PO autoriza → reviewer independente aprova → smg-merge → verificação pós-merge → novo STOP."* Autorização explícita do PO ("autorizado proximo passo") + `reviewDecision=APPROVED` por `review-bot-smg`; checks obrigatórios verdes antes do merge (validate/build/typecheck/unit/architecture:ci PASS; `lint advisory` FAIL = baseline pré-existente); head `3c43872` confirmado como ancestor de `main`. **Verificação pós-merge (worktree limpa de `origin/main`):** typecheck **exit 0** + build **exit 0** (14.81s). **Novo STOP:** deploy/homologação permanecem bloqueados até autorização explícita; drift B3 segue decisão futura. Branch: `feature/p0-4-abertura`. Registros: `PROJECT_STATUS.md` (7.16). |
| 2026-09-11 | 8.54 | **P0.3-C — Cancelar Cobrança ≠ Dar Baixa 🟢 CONCLUÍDA (2026-09-11) — correção do pipeline financeiro no fluxo novo de cancelamento ChefClub (gate `docs/audit/6.1.4_P0_3_STOP_GATE.md`, decisões D1–D7 do PO, 2026-09-11).** **D2 — correção de código:** `application/chefClub/operations.ts` — `cancelSubscriptionWithReceivables` passa a publicar `SubscriptionCancelled` (canal `appEventBus` → FinanceSubscriber → outbox/D8) somente quando `subscription_cancelled=true`, com `reason = cancelReason ?? 'user_cancelled'` e `metadata.source='ChefClubApplicationService'` (contrato `domain/events/types.ts` L156); caminho legado `cancelSubscription` preservado e com callers zero em páginas (re-exports/testes). **Achado estrutural:** matriz `createCommissionOnlyFinanceStrategy` mapeia `SubscriptionCancelled → NONE` — o evento agora alimenta EventStore/auditoria/consumo D8 futuro, **sem ativar `reverse_revenue`** (alteração de regra financeira ADR-001 = decisão separada do PO). **D7 — testes (10):** bloco "Grupo P0.3-C — Cancelamento com Recebíveis (D2)" em `application/chefClub/chefClub.test.ts` (L880) — cancelamento c/ e s/ recebíveis, publicação do evento, canal FinanceSubscriber (`subscribeAll`, mesma rota real), motivo obrigatório, zero recebíveis canceláveis, tenant propagation/rejeição, falha de autorização sem publicação, regressão legado. **Validação:** `npx tsc --noEmit` exit 0 · `npx vitest run` **1304 PASS / 0 FAIL** (67 arquivos) · `npm run build` exit 0 · `git diff --check` OK. **E2E STAGING (`tjcvuhynckocmvtqykxp`) 9/9 PASS** (flow-p0-3-c, AC-02..AC-15: Caminho A, Caminho B, AC-14 histórico, AC-13 tenant isolation, AC-11 atomicidade, AC-12 idempotência; teardown completo; runId `1789104229885`). **D3 — saneamento ACL controlado em PROD (`ushsnmlbeurfvlkieiln`):** REVOKE ALL ON FUNCTION de `service_role`+`anon` nas 2 RPCs (`cancel_club_receivables(UUID,UUID,TEXT,TEXT)`, `cancel_subscription_with_receivables(UUID,UUID,BOOLEAN,TEXT,TEXT)`); ACL antes `{postgres, service_role, authenticated}` → depois `{postgres, authenticated}` (verificação read-only); arquivo de saneamento **não commitado** (padrão M4). **D4 mantido 🔴:** nenhum INSERT/`migration repair` no ledger (`0001–0003` ausentes documentados como evidência para B3). **D6 STOP:** commit/push/PR executados; **merge NÃO executado** — aguarda autorização explícita do PO + reviewer independente (branch protection). Registros: `PROJECT_STATUS.md` (7.15), `docs/audit/6.1.4_P0_3_RELATORIO_FINAL.md`, `docs/audit/6.1.4_P0_3_STOP_GATE.md`. |
| 2026-09-11 | 8.53 | **M4/P7 — UI de registro de pagamento parcial/antecipado no Checkout 🟢 CONCLUÍDA + saneamento ACL das 4 funções M4 aplicado em PROD (2026-09-11, autorização formal do PO).** **PO (verbatim, 2026-09-11):** *"AUTORIZO o escopo M4 composto por: (1) saneamento ACL das 4 funções identificadas no preflight, incluindo get_comanda_payment_summary; (2) implementação do P7 — registro de pagamento parcial/antecipado no Checkout; (3) validação completa; (4) atualização da documentação/baseline. A aplicação da migration ACL em PROD fica autorizada dentro deste escopo, seguindo o ciclo governado e os gates de validação."* **ACL PROD:** migration `20260911120000_seguranca_revoke_anon_service_role_4_m4_functions.sql` aplicada em PRODUÇÃO (`ushsnmlbeurfvlkieiln`) — REVOKE EXECUTE de `anon`+`service_role` (+ REVOKE ALL FROM PUBLIC) e GRANT `authenticated` em `check_comanda_has_valid_payments` (M4-P1), `get_comanda_payment_summary` (M4-P7, novo achado preflight), `get_tenant_refund_method` + `upsert_tenant_refund_method` (M4-P8); verificado via Management API (proacl `{postgres, authenticated}` nas 4; antes `{postgres, anon, authenticated, service_role}`); `supabase migration repair --status applied`; evidências OID before/after no relatório M4. **UI P7:** `pages/Checkout.tsx` (+296) — estado/efeito com `getComandaPaymentSummary`, gate `canRegisterComandaPayment` (modo `edit_comanda` + comanda não paga/cancelada + recepção/gestão — espelha o gate do RPC, nunca barbeiro), card "Pagamentos registrados" (lista + Total pago + Restante) e modal "Registrar Pagamento" (toggle Antecipado/Parcial, valor ≤ restante, formas de pagamento + descrição para 'other', motivo opcional) via `registerComandaPayment` com validação e toasts pt-BR. **Validação completa:** `npx tsc --noEmit` **exit 0 — ZERO erros** · suíte completa `npx vitest run` **1294 PASS / 5 skipped / 0 FAIL** · `npm run build` ✔ (15.32s) · `npm run d8:verify` ✔ byte-identical (D8 intacto) · `git diff --check` OK. **MERGE EXECUTADO (registro pré-existente, 2026-09-11T03:47:47Z):** PR #33 merged em `main` via merge commit `1ce975f` (base `main`; `reviewDecision=APPROVED` por `review-bot-smg` no head `08df570`; CI pós-merge success — CI `34559788311` / Test `34559788378`). **Verificação pós-merge (worktree limpa de `origin/main`):** typecheck **exit 0** + build **exit 0** (14.26s). **E2E pendente:** suíte completa da baseline — previsto **14/09/2026** (aguarda autorização/lançamento do PO). Registros: `docs/audit/M4_RELATORIO_FINAL.md` (§7/§8/§10); `PROJECT_STATUS.md` (7.14). |
| 2026-09-10 | 8.52 | **P1.3 → P1.1 — MERGE AUTORIZADO (aguardando reviewer independente) + B3 READ-ONLY AUTORIZADO (decisões formais do PO, 2026-09-10).** **PO-6.1.4-B2-01** — *"autorizado o merge do PR #31, exclusivamente para consolidação documental do gate P1.3 → P1.1"* → merge **AUTORIZADO** (`gate/6.1.4-promocao-p1-3-p1-1` → `main`, PR #31: cadeia docs 6.1.4 A.1/B1/gate B + rollback D2/D3 + ROADMAP 8.51 + PROJECT_STATUS 7.12; zero código — par já em main via PRs #26/#29, migration `20260905000000` já aplicada em PROD). **PO-6.1.4-B2-03** — *"PRESERVAR BRANCH PROTECTION / DESBLOQUEAR VIA REVIEWER INDEPENDENTE"* → 🔴 **proibido contornar a proteção** (`--admin`, redução de review count, `enforce_admins` off, merge fora da proteção); desbloqueio **somente via reviewer independente** com write access (branch protection de `main` exige 1 approving review; account dono não pode aprovar PR próprio) → ⏸ **execução do merge AGUARDANDO reviewer**. **PO-6.1.4-B3-01** — *"após a confirmação do merge, autorizado iniciar B3 exclusivamente em modo read-only, para reconstrução e planejamento da proveniência/ledger PROD"* → 🟢 **B3 AUTORIZADO em READ-ONLY** (investigação + planejamento, zero escrita; início **após merge + STOP GATE**). **PO-6.1.4-B3-02** — *"expressamente não autorizado qualquer INSERT, migration, DDL, DML, GRANT/REVOKE ou alteração de produção durante essa investigação"* → 🔴 **proibição formal de escrita em produção durante o B3** (read-only estrito). **PO-6.1.4-B2-02** — *"M4/P0.3/P0.4/P2.1 ficam aguardando a conclusão/decisão do B3"* → ⏸ **B2 fase 2 BLOQUEADA** até o B3. Gate: `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§13/§14). |
| 2026-09-10 | 8.51 | **P1.3 → P1.1 PROMOVIDOS (gate de promoção 6.1.4-B2, 2026-09-10, autorização explícita do PO).** *"Fica autorizada a execução do gate de promoção P1.3/P1.1, mas não uma alteração arbitrária fora dela."* Fluxo cumprido: `smg-change-control → smg-isolate → smg-implement → smg-validate → smg-commit → smg-push → smg-pr → STOP → smg-merge`. **Descoberta estrutural:** código de P1.3/P1.1/D2/D3 já em `main` (PR #26 `cea99c9`, PR #29 `e32145a`, PR #30 `82e8dfc`); migration `20260905000000` final (`sep.staff_id` + REVOKE anon/service_role) já aplicada e validada em PROD (8/8 + smoke 7/7) — gate = **fechamento documental, zero código**. **Executado:** branch `gate/6.1.4-promocao-p1-3-p1-1` isolada de `origin/main`; `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` atualizado (header + §0 + nova §13 PROMOÇÃO FORMAL — P1.3/P1.1 **PROMOVIDOS**); evidências B2 (§12) preservadas (código inalterado desde a validação). **Validação:** `git diff --check` limpo; diff docs-only (zero migration/SQL/TS). **STOP:** merge do PR e deploy **NÃO executados — aguardam autorização explícita do PO** ("merge/deploy somente dentro do escopo P1.3/P1.1 e mediante as validações previstas no gate"). Pendências: B3 (ledger PROD, `20260905000000` com `remote=""`) não autorizado (sem INSERT); B2 fase 2 (M4, P0.3, P0.4, P2.1) somente após P1.3/P1.1. Registros: `PROJECT_STATUS.md` (7.12). |
| 2026-09-10 | 8.50 | **6.1.3 — Reconciliação Git × Supabase: REVOKE `anon`/`service_role`/`PUBLIC` das 9 funções críticas (D2) + gate cross-tenant `batch_unblock_comandas` (D3) + repair das 8 pendentes em staging (D4) + cert P2.1 (D5) + PRs de reconciliação (D6) 🟡 EXECUÇÃO CONTROLADA.** Rodada autorizada pelo PO (2026-09-10, decisões D1–D9 — STOP GATE 6.1.3, produção bloqueada até novo gate D7). **Executado em STAGING (`tjcvuhynckocmvtqykxp`):** (1) **D2** `20260910150000_seguranca_revoke_anon_public_service_role_9_functions.sql` — REVOKE ALL FROM PUBLIC + anon + service_role nas 9 funções (`reverse_comanda_payment`, `unblock_comanda`, `batch_unblock_comandas`, `pay_account_payable`, `cancel_account_payable`, `create_accounts_payable_from_recurring`, `create_one_time_account_payable`, `create_recurring_bill`, `import_clients_batch`) + GRANT `authenticated`; **aplicada + verificada** (`proacl` = `{postgres=X/postgres, authenticated=X/postgres}` em todas); (2) **D3** `20260910160000_seguranca_gate_batch_unblock_comandas.sql` — gate com `auth.uid()` + validação tenant/role gerencial + superadmin bypass no `batch_unblock_comandas` (única das 9 sem gate interno; conforme Evidência A); **aplicada + verificada**; (3) **D4** `migration repair --status applied` das 8 pendentes em staging (P0.3 mig1–4, P0.4 ×3, P2.1) — registro de estado, sem DDL (`schema_migrations` = 148 = 138 + 8 reparadas + 2 novas); (4) **D5** header da P2.1 (`20260906130000`) → "VERSÃO FINAL CERTIFICADA" (corpo idêntico ao deployado, diff UTF-8 limpo); (5) **D6** branches syncadas e pushadas (`feature/m4-finance-operations` `212cc02`, `feature/p1-3-canonical-kpis` `d449ec8`, `chore/seguranca-revoke-9-funcoes` `83d9a29`) + **PRs #28/#29/#30 → `main`** — CI `validate` **PASS** nos 3 (check obrigatório da branch protection); `lint advisory` FAIL = **baseline pré-existente** (33 erros/213 warnings em 96 arquivos TS/TSX, não relacionado às branches). **Produção INTOCADA** (6.1.4/6.1.5 🟥 BLOQUEADO). Evidências: `docs/audit/6.1.3_*`. Branch: `chore/seguranca-revoke-9-funcoes`. |
| 2026-09-06 | 8.49 | **P2.1 — Import Engine + CSV de Clientes 🟢 PRODUCTION CERTIFIED (2026-09-08).** Migration `20260906130000_p2_1_import_persistence.sql` aplicada em PRODUÇÃO (`ushsnmlbeurfvlkieiln`). Schema validado: 2 tabelas (`import_jobs`, `import_rows`), 1 RPC (`import_clients_batch` SECURITY DEFINER), 4 RLS policies (tenant_isolation + superadmin_bypass), 7 indexes (incl. UNIQUE idempotency), EXECUTE grant para `authenticated`. Smoke test funcional via SQL direto: schema integrity confirmed. Produção: 364 clients existentes intactos, zero impacto em dados. Branch: `feature/p2-1-csv-clientes` merged to `main` (PR #27, SHA `66922f4`). Baseline: `v2.1.0-p2.1-homologation-pass`.
| 2026-09-06 | 8.48 | **P1.3 — KPIs Canônicos via RPC `get_dashboard_kpis` 🟢 CLOSED (2026-09-06, autorização formal do PO).** *"AUTORIZO O FECHAMENTO DA P1.3 — KPIs CANÔNICOS."* Migration `20260905000000` aplicada e validada em produção (`ushsnmlbeurfvlkieiln`) — **8/8 critérios pós-produção atendidos + smoke 7/7 PASS**. **Correção do drift:** K9 `sep.professional_id` → `sep.staff_id` (2 posições, mesma FK `public.staff(id)`, alias de saída `professional_id` preservado — contrato `kpiTypes.ts` inalterado) + `REVOKE EXECUTE ... FROM anon, service_role` (higiene de grants). **Revalidação staging (Opção A, autorização prévia do PO):** schema alinhado (alignment `20260906000000`: `ADD COLUMN staff_id` + backfill guardado + índice), ACL anon=false/sr=false/auth=true, prosrc zero refs `professional_id`, **27/27 testes PASS** (13 equivalência + 14 segurança), **E2E estrito PASS** (card "Faturamento" via RPC real, sem banner degradação). **Produção:** aplicada via Management API (HTTP 201, `CREATE OR REPLACE`), validação pós-produção 8/8 (assinatura `get_dashboard_kpis(text,uuid)` SECURITY DEFINER jsonb; prosrc `sep.staff_id` pos 10450; zero `sep.professional_id`; ACL authenticated=true/anon=false/service_role=false; schema/índice SEP compatível; **zero DML/DLL de tabela** na migration); **smoke RPC produção 7/7 PASS** (tenant 100% efêmero + teardown FK-aware verificado: unscoped 200, scoped 200, anon 401 `permission denied for function get_dashboard_kpis`). **Alignment `20260906000000` NÃO aplicada em produção** (decisão PO: no-op desnecessário — schema produtivo já alinhado). **Nenhuma reconciliação/alteração adicional de dados autorizada.** Commit `9747a72` + `9b977aa`. Gate: `docs/audit/P1_3_PRODUCTION_GATE.md` (§9.1). Estado consolidado P1: **P1.1 🟢 CLOSED · P1.3 🟢 CLOSED · P1.2 permanece conforme roadmap (não inferir concluída)**. |
| 2026-09-06 | 8.47 | **P1.1 — Central de Relatórios: aba Vendas 🟢 ENCERRADA (2026-09-06, merge autorizado pelo PO).** PR #26 merged (`feature/p1-1-central-de-relatorios` → `feature/p1-3-canonical-kpis`, **merge `cea99c9`**, 8 arquivos, +684/−32). **Homologação staging:** migration P1.3 `20260905000000_get_dashboard_kpis.sql` aplicada **exclusivamente no staging** (`tjcvuhynckocmvtqykxp`, autorização explícita do PO) + registro em `supabase_migrations.schema_migrations`; RPC confirmado (`to_regprocedure` OK, `authenticated` EXECUTE); E2E estrito contra staging PASS — card "Faturamento" via RPC real, sem banner de degradação. **Validação pós-merge em `feature/p1-3-canonical-kpis`:** typecheck 72 = 72 pré-existentes (zero novos); **build PASS (14.06s, EXIT 0)**. **Produção NÃO tocada** — migration P1.3 **não** aplicada em produção (STOP mantido, autorização separada do PO). Próximo gate: autorização do PO para merge/avanço em produção. |
| 2026-09-06 | 8.46 | **P1.1 — Central de Relatórios: aba Vendas 🟡 IMPLEMENTADO (2026-09-06).** Branch `feature/p1-1-central-de-relatorios`, design gate `docs/audit/P1_1_DESIGN_GATE.md` (PO APPROVAL). Aba Vendas em `pages/Reports.tsx`: cards canônicos **exclusivamente** via RPC `getDashboardKpis()` (zero recálculo local — §4.4/§4.5), detalhamento por comandas com intervalo livre + join `clients(name)` (§2), mapeamento preset→período canônico em `src/modules/reports/presetMapping.ts` (§5: today/yesterday/this_month/this_year traduzem; last_7_days/last_month/custom **ancorados** no último canônico renderizado), rótulo anti-misleading "Período dos KPIs / Detalhamento" para presets não mapeados (§6), aba ChefClub intacta. **Testes:** 13/13 unit mapeamento + 27/27 regressão P1.3 (equivalência + segurança); typecheck sem erros novos (72 pré-existentes); build PASS. **Migration P1.3 NÃO aplicada** — cards degradam graciosamente (banner + listagem local continua). Staging validation + merge exigem autorização separada do PO. |
| 2026-09-05 | 8.45 | **P1.3 — KPIs Canônicos via RPC `get_dashboard_kpis` 🟡 IMPLEMENTADO (2026-09-05).** Branch `feature/p1-3-canonical-kpis`, commit `d10d9bb`. Migration `20260905000000_p1_3_get_dashboard_kpis.sql` — PL/pgSQL SECURITY DEFINER com 5 gates de segurança (auth/tenant/role/staff scope/period) + superadmin bypass. TypeScript client (`src/modules/dashboard/rpc.ts`) + types (`kpiTypes.ts`). **Testes:** 13 equivalência (old `buildDashboardMetrics` vs SQL, \|Δ\| ≤ 0.01) + 14 segurança (GATE 1-5, role validation, tenant isolation, parameter passing) = **27/27 PASS**. Decisões: D-EST-01 (reversões deduzem receita), D-PERF-01 (server-side SECURITY DEFINER), D-RET-01 (retenção por `status='completed'`). **Migration NÃO aplicada em staging/produção** (exige STOP gate separado do PO). Entry audit: `docs/audit/P1_3_ENTRY_AUDIT.md`. |
