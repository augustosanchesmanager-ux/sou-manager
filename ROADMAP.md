...
- [ ] **6.0.5** Billing/Tenant/Feature Flags — Arquitetura congelada pelo **ADR-013** (3 contextos desacoplados + Estado Efetivo): ~~6.0.5.1 camada de autorização~~ (✅ concluída), ~~6.0.5.2 BillingService + Modelagem de Plans~~ (✅ concluída — deploy pendente), ~~6.0.5.3 FeatureFlagService~~ (⏳ relatório final submetido em 07/08 — aguardando aprovação do PO para implementação), 6.0.5.4 TenantLifecycleService + `suspended` aditivo, 6.0.5.5 transições RPCs
