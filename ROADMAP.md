...
> **⚠️ Descoberta de validação (§12.5):** RPCs irmãs das migrations `20260806020000`/`20260806050000`/`20260807010000` compartilham o mesmo bug latente de referência ambígua (`RETURNS TABLE` OUT param × coluna não qualificada) — **nunca executadas em Postgres real** (banco local sem `subscriptions`). Fix aditivo recomendado no runbook/janela única (requer decisão do PO).
...
- [x] 6.0.5.5 concluída (implementação + unit 883/883 + migration `20260807020000` T1–T12 OK em docker + **SCHEMA FREEZE = YES**; E2E flow11 adiado — decisão PO 2026-08-07)
