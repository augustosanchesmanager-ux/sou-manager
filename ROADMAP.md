...
#### 6.0.5.5 — Transições RPCs (`change_tenant_plan` + banner + `UpgradePrompt` + correção `Admin.tsx`) ⏳ PLANNED

> **Entry audit submetida em 2026-08-07 (decisão do PO) — última implementação funcional da série 6.0.5.**
> Documento: `docs/audit/PHASE_6_0_5_5_ENTRY_AUDIT.md`.
...
**Escopo (D-6.0.5.5-2 proposto):**
- RPC `change_tenant_plan` (upgrade/downgrade transacional; grava `subscriptions.plan` + espelho `tenants.plan`; grants ADR-012) — migration planejada `20260807020000_phase_6_0_5_5_transitions.sql`;
- `application/tenantLifecycle.ts`: método `changePlan` + evento `TenantSubscriptionUpdated`;
- correção `pages/Admin.tsx:856` (remoção da escrita direta de `tenants.plan`);
...
- depreciação de `domain/authorization/featureAvailability.ts` (fora do runtime).
...
- **Veredito preliminar (§3 da entry audit): `SCHEMA FREEZE = NO`** — o schema ainda mudará em **1 objeto**: nova RPC `change_tenant_plan`. Nada mais: sem tabelas/colunas/FKs/policies; assinaturas de RPCs existentes intocadas; mudança semântica de `tenants.plan` (escrita UI → espelho) sem alteração de schema.
- **Fechamento:** gate **reexecutado** com o diff real → **`SCHEMA FREEZE = YES`** registrado aqui e no `RELEASE_CHECKLIST_v1.5.md` **antes** da PCA (6.0.5.6).

**Critério de entrada:** aprovação do PO da entry audit (D-6.0.5.5-1..5) + confirmação do hardening opcional M7/M11/M12 + E2E flow11 (D-6.0.5.5-4).
