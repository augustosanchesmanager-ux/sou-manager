...
|------|--------|-----------|
| 2026-09-06 | 8.45 | **P2.1 — Import Engine + CSV de Clientes 🟢 IMPLEMENTADO (2026-09-06).** Engine completo em `src/modules/import-engine/` (types, CSV parser, pipeline validação/preview, definitions, index + 24/24 testes unitários). Integração em `pages/Clients.tsx`: upload CSV, preview com detecção de duplicatas por telefone (heurística, não bloqueante), linhas válidas/inválidas, confirmação com idempotency key + `importViaJob`. Types: `domain/client/types.ts` (`ClientImportRow`, `ImportJobResult`); Repository: `domain/client/repository.ts` (`importViaJob` + RPC `import_clients_batch` transacional). Typecheck: 72 pré-existentes, zero novos (fix `Set<string>` explícito em `existingKeys` — causa raiz: `@types/react` ausente, `useState` resolve como `any`). Testes: 1196 passed / 3 failed / 5 skipped (falhas h2-8 staging pré-existentes). Build: PASS. **Sem migration/RPC** — persistência via RPC transacional gateado por decisão separada do PO (princípio P2: duplicidade sempre heurística). Branch: `feature/p2-1-csv-clientes`.
