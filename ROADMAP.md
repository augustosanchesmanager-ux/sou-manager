...
#### 6.0.5.6 — Production Compatibility Audit (PCA) ❌ BLOCKED

> **Registrada em 2026-08-07 (decisão do PO) como etapa oficial da release v1.5.**
> **Executada em 2026-08-08 (somente leitura) → veredito `BLOCKED`.**
...
**Resultado da execução (2026-08-08):** ❌ **`BLOCKED`** — auditoria somente leitura do banco real (project ref `ushsnmlbeurfvlkieiln`) concluída. **1 incompatibilidade crítica:** a migration `20260806030000` foi **pulada no remoto** e o `cancel_subscription` atual (11 colunas, definido por `06050000`/`06070000` já aplicadas) é **incompatível** com o `CREATE OR REPLACE` de 5 colunas da pendente → **erro garantido na janela única**. Correção exige decisão do PO (`migration repair --status applied 20260806030000` — recomendado, pois a autorização que ela adiciona já está no remoto via `06070000`; ou migration corretiva). **3 incompatibilidades de dados:** Barbearia Principal (produtivo real, `free` com 4 staff ativos), Loja Demo Varejo (`free`, 3) e SMG Estética (`free`, 2) excedem `max_staff=1` — decisão de negócio do PO. Demais seções (tenants, plans, subscriptions, billing, chef club, segurança, integridade) **compatíveis**. **Gate de release mantido: nenhuma migration de produção sem PCA = READY** — ver `docs/audit/PRODUCTION_COMPATIBILITY_AUDIT.md`.
...
| 2026-08-08 | 8.15 | **6.0.5.6 — Production Compatibility Audit (PCA) EXECUTADA → ❌ `BLOCKED` (2026-08-08, somente leitura).** Auditoria do banco real dos tenants produtivos (project ref `ushsnmlbeurfvlkieiln` — Sanchez Barber) concluída via consultas individuais no runner Management API (Supabase CLI v2.105.0; runner não aceita meta-comandos psql e retorna apenas o último result set de arquivos multi-statement). **1 incompatibilidade crítica de migration + 3 excedências de limite de plano.** **Bloqueio crítico:** migration `20260806030000` **pulada no remoto** — `cancel_subscription` atual com **11 colunas** (`cancel_at_period_end`, definida por `06050000`/`06070000` **já aplicadas**) × `CREATE OR REPLACE` de **5 colunas** da pendente → PostgreSQL rejeita (`cannot change return type of existing function`) → **erro garantido na janela única**; a `06030000` também regrediria a semântica do BillingEngine 6.0.4.4 (pedido → efetivo). A autorização que ela adiciona **já está no remoto** via `06070000` (`current_is_tenant_manager_from_auth_uid`). **3 opções de correção documentadas (decisão do PO):** (1) `supabase migration repair --status applied 20260806030000` — **recomendada**; (2) nova migration corretiva com `DROP FUNCTION` prévio + recriação na assinatura de 11 colunas; (3) manter a `06030000` sem aplicar (desvio documentado). **Incompatibilidades de dados:** Barbearia Principal (produtivo real, `free`, **4 staff** ativos), Loja Demo Varejo (`free`, 3) e SMG Estética (`free`, 2) excedem `max_staff=1` — decisão de negócio do PO (upgrade `pro` ou legado aceito). **Compatível:** 45 tenants, 0 sem plano, 0 planos inválidos, status todos no enum; **1 subscription** no banco (E2E Flow9) — 0 órfãs, 0 duplicadas; `plans`/`features`/`plan_features` ausentes (esperado — criados pela `06090000`); invoices 0 / billing_events 2 / payment_attempts 0; 15 assinaturas Chef Club + 42 receivables; 16 RPCs auditadas (assinaturas compatíveis; `apply_subscription_transition`/`get_due_subscriptions` usam DROP+CREATE na `07010000`); FKs íntegras; RLS/RPC grants aderentes à ADR-012. Veredito detalhado: `docs/audit/PRODUCTION_COMPATIBILITY_AUDIT.md`. **Gate mantido: nenhuma migration de produção sem PCA = READY.** |
