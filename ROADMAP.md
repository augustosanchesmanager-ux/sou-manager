...
|------|--------|-----------|
| 2026-08-25 | 8.30 | **TD-001 B3.4-H — baseline `78cf2bc` CERTIFICADA pelo PO (`v1.4.4-commission-reversal-certified`) após validação funcional ponta-a-ponta da pipeline de comissão em produção.** Deploy promovido via integração Git/Vercel `dpl_9WLpjBn255FQBA3w69znaPwtWwd2` (READY, alias `barber.soumanager.com`) servindo o escopo `fb5403a..78cf2bc` (17 commits com SUT financeiro real: repos/RPCs de `commission_records`, handlers FinanceProvider, FinanceStrategy, event infrastructure); marcadores de bundle confirmados (`reversal-*.js` com `finance_reverse_transaction`, `create_commission_record`, `CheckoutReverted`, `processed_operations`; `CommissionCalculated` ausente por decisão B3.4-G commission-only). **Ciclo #1 (FAIL do ciclo/inconclusivo p/ produto):** comanda `58ddb28e…` pagou R$15 mas o item persistiu `staff_id=''` apesar da UI exibir a primeira conta → guard `if (staffId && total>0)` (`defaultFinanceStrategy.ts:86`) skipou a comissão **por design** (sem pagamento indevido); estorno de transação OK — **known issue** de atribuição de responsável (`pages/Checkout.tsx:948` default `staff[0]`). **Ciclo #2 (PASS WITH REVERSAL):** tenant Sanchez Barber `b716e290…`, HERON FERREIRA `62ddf002…` (50%), serviço R$15 → comanda `f859260a` paid → `commission_records` **+R$7,50** idempotente (`processed_operations` 1:1) → reversão auditada **−R$7,50** vinculada ao original → líquido zero; par income/expense/full_refund íntegro; multi-tenant isolado. Repetibilidade comprovada com 6 registros reais adicionais (22:55–22:59 UTC, R$130,00 total). Ledger final: 8 registros (7+/1−), líquido R$130,00, 0 duplicidade, 0 cross-tenant. **Decisão PO: R$130 preservados como dado real** (estorno futuro somente via UI). **Known issue → Trilha B em branch separada (backlog prioritário)** — spec: cadeia seleção→estado→payload→persistência do responsável + teste de regressão + preview antes de produção; pipeline financeiro intocado. Zero código/migration/RLS/RPC/env alterados; banco somente leitura. Evidência: `docs/audit/TD001_B3_4_H_PRODUCTION_CERTIFICATION.md`. Registros: `PROJECT_STATUS.md`. |
