...
|------|--------|-----------|
| 2026-09-24 | 8.77 | **H3-5 + P1-01 — frente ENCERRADA · PROD HEALTH GATE = PASS (decisão PO 2026-09-24).** Cadeia formal completa: `smg-change-control` → `smg-isolate` → `smg-implement` → `smg-validate` (staging E2E **8/8 PASS**) → `smg-commit` (`5ad3c46`) → `smg-push` → `smg-pr` (#79) → PO `/approve` → `review-bot-smg` APPROVED → **`smg-merge` = `5a28456`** (2026-09-24T06:47:33Z) → **post-deploy audit read-only PASS**. **Evidências PROD:** Vercel Production success @ `5a28456`; tree == `5ad3c46`; `barber.soumanager.com` 200; bundle `index-DD_frCHM.js` → só Supabase PROD `ushsnmlbeurfvlkieiln`; rotas login/dashboard 200; **`comanda_items.staff_id` existe no PROD** (SELECT REST 200); CI pós-merge obrigatória verde (`lint advisory` = baseline). **Side-effect disclose:** merge em `main` **auto-disparou** Vercel Production (`vercel[bot]`, 06:48:10Z) — processo desenha merge≠deploy; infra não. **Nenhuma alteração adicional em PROD autorizada.** **3 pendências INDEPENDENTES (novas frentes, `smg-change-control` próprio; não implementadas nesta noite):** (1) `deploy-validate.yml` — `if` exige `deployment.ref == 'refs/heads/main'` mas Vercel envia `ref=<SHA>` → workflow **skipped** quando deveria validar pós-deploy; (2) migration `20260924000000_add_comanda_items_staff_id.sql` — **untracked**, **não aplicada em PROD** (no-op: coluna já existe); decisão própria = versionamento + drift STAGING; (3) **dívida change-control** — auto-deploy Production no merge vs processo MERGE→PO decide DEPLOY. **Não misturar** com pipeline/migration/governança. Registros: `PROJECT_STATUS.md` (7.38), `docs/BUSINESS_DECISIONS.md` (D-HOM-32), `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md` (§10.7 pós-frente). |
