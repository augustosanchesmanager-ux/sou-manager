...
>
> **Registro da dívida D8 (2026-08-27, Post-Deploy + Chaos gate do ADR-015):** dependência client-side do dispatcher outbox — ver tabela acima. Decisão do PO: (1) **não certificar ADR-015 em produção** até drenar a comissão pendente `63742efa` via sessão autenticada legítima do tenant (sem forjar/bypassar auth/RLS); (2) registrar como dívida arquitetural P1/P2; (3) **não abrir frente server-side agora** (uma trilha por vez). **Somente documentação + commit do harness chaos — nenhum código de produção alterado.**
