...
|------|--------|-----------|
| 2026-09-02 | 8.43 | **H-8 (Homologação) 🟢 PASS COM CONDIÇÃO — veredito formal do PO (D-HOM-28, 2026-09-02) + retrofit ACL em trilha própria (`chore/seguranca-acl-retrofit-3-rpcs`).** Revalidação **read-only** contra a produção `ushsnmlbeurfvlkieiln`: **premissa D-HOM-10 obsoleta** — produção `smg-barber` em **`c1c40da`** (FASE 6 completa; deploy automático pós-merge `2ce5a7f`; bundle `/assets/index-DXO2_Rnd.js` com código FASE 6); **4 migrations corretivas verificadas como aplicadas** (assinaturas tenant-scoped + guardas `auth.uid()`/`current_tenant_id_from_auth_uid`/membership/superadmin + MD5). **Condição:** ACL residual (`anon`/`service_role`) em `correct_appointment_attendance`/`register_comanda_payment`/`confirm_appointment_attendance` — migrations corretivas revogam só `FROM PUBLIC` (as bulk_close revogam `PUBLIC, anon, service_role`); guarda interna mitiga em runtime. **Decisões PO (verbatim):** manter `c1c40da` **SIM** (rollback **NÃO**) · **criar retrofit das 3 ACLs SIM** → migration `20260902000000_seguranca_fix_acl_tenant_scoped_rpcs.sql` (REVOKE `PUBLIC, anon, service_role` + GRANT `authenticated` + `NOTIFY pgrst`) · aplicar em produção imediatamente **NÃO** · **homologar antes de aplicar SIM** · `smg-estetica` (terceiro projeto Vercel, mesmo repo) registrado para trilha própria (não bloqueia). **H-8 encerrado — gate não reaberto.** `get_comanda_payment_summary` residual fora do escopo (observação para trilha futura). Próximo bloqueador real: H-7 (S3-1 R$260 RIOS + janela acompanhada). Registros: `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-28), `docs/BUSINESS_DECISIONS.md` (D-HOM-28), `PROJECT_STATUS.md` (7.4). |
| 2026-09-02 | 8.42 | **STAGING GATE FASE 6 — PROMOÇÃO/​VERSIONAMENTO CONCLUÍDO (2026-09-02).** Merge autorizado pelo PO (`chore/seguranca-bulk-close-comandas-admin` → `main`): **merge `2ce5a7f`** (`--no-ff`, ort, sem conflitos; 79 arquivos, +12040/−12). Validação pós-merge em `main`: **build PASS (17.25s, EXIT 0)**; **typecheck 65 = 65 pré-existentes, **zero novos erros** introduzidos pelo merge (conjunto idêntico ao tip da branch). **Baseline: tag anotada `v2.1.0-staging-gate-approved`** no commit pós-merge `2ce5a7f` (decisão do PO: certificar o **estado promovido em main**). Push: `main` (`d4de4f8..2ce5a7f`) + tag `v2.1.0-staging-gate-approved`. Migrations corretivas `20260831120000`/`20260901120000`/`20260901150000`/`20260901160100` **NÃO aplicadas em produção** (decisão própria separada — autorização independente do PO). Próximo gate formal: Homologação Sanchez Barber (D-HOM) → Fase 6.0.6 Compliance & Legal → certificação Release v1.5. |
