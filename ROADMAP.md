...
|------|--------|-----------|
| 2026-09-10 | 8.52 | **P1.3 → P1.1 — MERGE AUTORIZADO E EXECUTADO + B3 READ-ONLY AUTORIZADO (decisões formais do PO, 2026-09-10).** **PO-6.1.4-B2-01** — *"autorizado o merge do PR #31, exclusivamente para consolidação documental do gate P1.3 → P1.1"* → **merge executado** (`gate/6.1.4-promocao-p1-3-p1-1` → `main`, PR #31: cadeia docs 6.1.4 A.1/B1/gate B + rollback D2/D3 + ROADMAP 8.51 + PROJECT_STATUS 7.12; zero código — par já em main via PRs #26/#29, migration `20260905000000` já aplicada em PROD). **PO-6.1.4-B3-01** — *"após a confirmação do merge, autorizado iniciar B3 exclusivamente em modo read-only, para reconstrução e planejamento da proveniência/ledger PROD"* → 🟢 **B3 INICIADO em READ-ONLY** (investigação + planejamento, zero escrita). **PO-6.1.4-B3-02** — *"expressamente não autorizado qualquer INSERT, migration, DDL, DML, GRANT/REVOKE ou alteração de produção durante essa investigação"* → 🔴 **proibição formal de escrita em produção durante o B3** (read-only estrito). **PO-6.1.4-B2-02** — *"M4/P0.3/P0.4/P2.1 ficam aguardando a conclusão/decisão do B3"* → ⏸ **B2 fase 2 BLOQUEADA** até o B3. Gate: `docs/audit/6.1.4_B_STOP_GATE_CLASSIFICACAO.md` (§13/§14). |
