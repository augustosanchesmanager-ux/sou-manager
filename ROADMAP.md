...
|------|--------|-----------|
| 2026-09-02 | 8.43 | **H-8 (Homologação) 🟢 PASS COM CONDIÇÃO → PRODUÇÃO APLICADA → AUDITORIA PASS — retrofit ACL das 3 RPCs (D-HOM-28 + D-HOM-29 + D-HOM-29b, 2026-09-02).** Revalidação read-only da produção `ushsnmlbeurfvlkieiln`: premissa D-HOM-10 obsoleta — `smg-barber` em `c1c40da` (FASE 6); 4 migrations corretivas verificadas. Condição: ACL residual em 3 RPCs. Staging homologado (`tjcvuhynckocmvtqykxp`): migration aplicada, 3/3 PASS. **Produção aplicada** (`ushsnmlbeurfvlkieiln`): PO autorizou; `confirm_appointment_attendance`, `correct_appointment_attendance`, `register_comanda_payment` — `anon`/`service_role` revogados, `authenticated` mantido. **Auditoria oficial do PO: 9,8/10 PASS** — `get_comanda_payment_summary` intocada (fora do escopo, trilha própria futura, não é blocker). **H-8彻底 encerrado.** Próximo gate: **H-7 — Operação Real acompanhada** (foco reconciliação S3-1 R$260 RIOS como finding a investigar durante a janela). Registros: `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-28/29/29b), `docs/BUSINESS_DECISIONS.md` (D-HOM-28/29), `PROJECT_STATUS.md` (7.4). |
