...
|------|--------|-----------|
| 2026-09-06 | 8.45 | **P2.1 — Import Engine + CSV de Clientes 🟢 HOMOLOGATION PASS (2026-09-07).** Engine completo em `src/modules/import-engine/` (types, CSV parser, pipeline validação/preview, definitions, index + 24/24 testes unitários). Integração em `pages/Clients.tsx`: upload CSV, preview com detecção de duplicatas por telefone (heurística, não bloqueante), linhas válidas/inválidas, confirmação com idempotency key + `importViaJob`. Types: `domain/client/types.ts` (`ClientImportRow`, `ImportJobResult`); Repository: `domain/client/repository.ts` (`importViaJob` + RPC `import_clients_batch` transacional). Migration `20260906130000_p2_1_import_persistence.sql` aplicada em staging (`tjcvuhynckocmvtqykxp`): tabelas `import_jobs`/`import_rows`, RPC `import_clients_batch` (SECURITY DEFINER, idempotente, SEC-01 race condition fix, SEC-02 classified errors, SEC-03 DoS limits 5000/5MB), RLS tenant_isolation + superadmin_bypass, 7 indexes incl. UNIQUE idempotency. **12 critérios validados: PASS.** **E2E real staging: PASS** — RPC funcional com autenticação JWT (#4 persistência, #5 idempotência, #6 retry sem duplicação, #7 concorrência sem duplicação, #12 auditoria). Typecheck: 72 pré-existentes, zero novos. Build: PASS. Branch: `feature/p2-1-csv-clientes`.
