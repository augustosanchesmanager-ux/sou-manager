...
const Clients: React.FC = () => {
    ...
    const openClientDetails = useCallback(async (client: Client) => {
        ...
        if (subscriptionError) {
            console.error('Erro ao carregar detalhes Chef Club:', subscriptionError);
            setDetailChefClub(null);
            return;
        }
        ...
        if (data) {
            ...
            if (clientIds.length > 0) {
                ...
                if (subs) {
                    ...
                    const plansById = (plans || []).reduce<Record<string, { id: string; name: string }>>((acc, plan) => {
                        acc[plan.id] = plan;
                        return acc;
                    }, {});
                    ...
                    subs.forEach((subscription: any) => {
                        map[subscription.client_id] = plansById[subscription.plan_id]?.name || 'Plano ativo';
                    });
                    setChefClubMap(map);
                }
            } else {
