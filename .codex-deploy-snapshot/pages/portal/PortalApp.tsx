...
const PortalApp: React.FC = () => {
    ...
    const loadPortalData = async () => {
        try {
            ...
            const [{ data: upcomingRows, error: upcomingError }, { data: historyRows, error: historyError }] = await Promise.all([
                supabase
                    .from('appointments')
                    .select('id, start_time, end_time, status, service_id, staff_id')
                    .eq('tenant_id', tenantData.id)
            ...
            const rawAppointments = [
                ...((upcomingRows || []) as PortalAppointment[]),
                ...((historyRows || []) as PortalAppointment[]),
            ];
            ...
            const [servicesResult, staffResult] = await Promise.all([
                serviceIds.length > 0
                    ? supabase
                        ...
                        .in('id', serviceIds)
                    : Promise.resolve({ data: [] as AppointmentService[], error: null }),
                staffIds.length > 0
                    ? supabase
                        .from('staff')
                        .select('id, name')
                        .in('id', staffIds)
                    : Promise.resolve({ data: [] as AppointmentStaff[], error: null }),
            ]);

            if (servicesResult.error) throw servicesResult.error;
            if (staffResult.error) throw staffResult.error;

            const servicesById = ((servicesResult.data || []) as AppointmentService[]).reduce<Record<string, AppointmentService>>((acc, service) => {
                acc[service.id] = service;
                return acc;
            }, {});
            const staffById = ((staffResult.data || []) as AppointmentStaff[]).reduce<Record<string, AppointmentStaff>>((acc, staffMember) => {
                acc[staffMember.id] = staffMember;
                return acc;
            }, {});

            const hydrateAppointment = (appointment: PortalAppointment): PortalAppointment => ({
                ...appointment,
                service: appointment.service_id ? servicesById[appointment.service_id] || null : null,
                staff: appointment.staff_id ? staffById[appointment.staff_id] || null : null,
            });

            const upcoming = (upcomingRows || [])[0] as PortalAppointment | undefined;
            setUpcomingAppt(upcoming ? hydrateAppointment(upcoming) : null);
            setPastAppts(((historyRows || []) as PortalAppointment[]).map(hydrateAppointment));
