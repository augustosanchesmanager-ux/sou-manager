...
const Suppliers: React.FC = () => {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const currentAppSlug = ensureAppSupportsModule(appSlug, 'suppliers', ['barber']);
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      category: trimmedForm.category || null,
      document: trimmedForm.document || null,
      address: trimmedForm.address || null
    };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { tenantId: resolvedTenantId } = requireTenantContext({
        ...
        schema,
        table: 'suppliers',
        operation: editingSupplier ? 'update supplier' : 'create supplier'
      });
      const client = getScopedClient(currentAppSlug);
      ...
      if (editingSupplier) {
        ...
        const { error } = await client
          .from('suppliers')
          .insert([{ ...payload, tenant_id: resolvedTenantId }]);
      ...
      setToast({ message: editingSupplier ? 'Erro ao atualizar fornecedor.' : 'Erro ao cadastrar fornecedor.', type: 'error' });
    }
  };
