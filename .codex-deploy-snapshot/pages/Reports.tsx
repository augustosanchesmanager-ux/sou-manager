...
const Reports: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const { data: paidComandas, error: paidComandasError } = await supabase
                .from('comandas')
                .select('id, total, client_id, staff_id')
                .eq('status', 'paid')
                .eq('tenant_id', tenantId);

            if (paidComandasError) {
                throw paidComandasError;
            }

            if (paidComandas) {
                const comandaIds = paidComandas.map((comanda: any) => comanda.id);
                const paidStaffIds = Array.from(new Set(
                    paidComandas
                        .map((comanda: any) => comanda.staff_id)
                        .filter((staffId: string | null | undefined): staffId is string => Boolean(staffId))
                ));

                const [itemsResult, paidStaffResult] = await Promise.all([
                    comandaIds.length > 0
                        ? supabase
                            .from('comanda_items')
                            .select('comanda_id, product_name, unit_price, quantity')
                            .eq('tenant_id', tenantId)
                            .in('comanda_id', comandaIds)
                        : Promise.resolve({ data: [], error: null }),
                    paidStaffIds.length > 0
                        ? supabase
                            .from('staff')
                            .select('id, name')
                            .eq('tenant_id', tenantId)
                ...
                if (itemsResult.error || paidStaffResult.error) {
                    throw itemsResult.error || paidStaffResult.error;
                }

                const itemsByComandaId = (itemsResult.data || []).reduce<Record<string, Array<{ product_name: string; unit_price: number; quantity: number }>>>((acc, item) => {
                ...
                const paidStaffById = (paidStaffResult.data || []).reduce<Record<string, { id: string; name: string }>>((acc, staffMember) => {
                ...
                paidComandas.forEach((com: any) => {
                    ...
                    // Staff performance
                    const staffName = com.staff_id ? (paidStaffById[com.staff_id]?.name || 'Venda Direta') : 'Venda Direta';
                    ...
                    (itemsByComandaId[com.id] || []).forEach((item: any) => {
