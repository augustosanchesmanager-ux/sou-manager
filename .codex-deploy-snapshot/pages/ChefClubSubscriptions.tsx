...
const ChefClubSubscriptions: React.FC = () => {
    ...
    const fetchSubscriptions = async () => {
        ...
        const { data, error } = await supabase
            ...
            .select('id, client_id, plan_id, status, cycle_end, next_billing_date')
        ...
        if (data) {
            ...
            const planIds = Array.from(new Set(
                data
                    ...
                    .filter((planId: string | null | undefined): planId is string => Boolean(planId))
            ));
            const subscriptionIds = data.map((subscription: any) => subscription.id);

            const [clientsResult, plansResult, creditsResult] = await Promise.all([
                clientIds.length > 0
                    ? supabase
                        .from('clients')
            ...
            if (clientsResult.error || plansResult.error || creditsResult.error) {
                setToast({ message: 'Erro ao carregar detalhes das assinaturas.', type: 'error' });
                setLoading(false);
                return;
            }

            const clientsById = (clientsResult.data || []).reduce<Record<string, { id: string; name: string; phone: string }>>((acc, client) => {
                acc[client.id] = client;
                return acc;
            }, {});
            const plansById = (plansResult.data || []).reduce<Record<string, { id: string; name: string; service_credits: number }>>((acc, plan) => {
                acc[plan.id] = plan;
                return acc;
            }, {});
            const creditsBySubscriptionId = (creditsResult.data || []).reduce<Record<string, number>>((acc, credit) => {
                acc[credit.subscription_id] = credit.available_credits || 0;
                return acc;
            }, {});

            const formatted: Subscription[] = data.map((subscription: any) => ({
                id: subscription.id,
                client_id: subscription.client_id,
                plan_id: subscription.plan_id,
                client: subscription.client_id && clientsById[subscription.client_id]
                    ? { name: clientsById[subscription.client_id].name, phone: clientsById[subscription.client_id].phone }
                    : { name: 'Cliente', phone: '' },
                plan: subscription.plan_id && plansById[subscription.plan_id]
                    ? { name: plansById[subscription.plan_id].name, service_credits: plansById[subscription.plan_id].service_credits }
                    : { name: 'Plano', service_credits: 0 },
                status: subscription.status,
                cycle_end: subscription.cycle_end,
                next_billing_date: subscription.next_billing_date,
                available_credits: creditsBySubscriptionId[subscription.id] || 0
            }));
