...
const Orders: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const currentAppSlug = ensureAppSupportsModule(appSlug, 'orders', ['barber']);
            const { tenantId: resolvedTenantId } = requireTenantContext({
                tenantId,
                appSlug: currentAppSlug,
                schema,
                table: 'purchase_orders',
                operation: 'load purchase orders',
            });
            ...
            console.error('Error loading orders module:', error);
            setToast({ message: 'Erro ao carregar pedidos e cadastros auxiliares', type: 'error' });
            setOrders([]);
            setSuppliers([]);
            setProducts([]);
        }
    ...
    const handleUpdateStatus = async (orderId: string, newStatus: PurchaseOrder['status']) => {
        try {
            ...
            const { tenantId: resolvedTenantId } = requireTenantContext({
                ...
                schema,
                table: 'purchase_orders',
                operation: 'update purchase order status',
            });
            const client = getScopedClient(currentAppSlug);
    ...
    const handleSaveSupplier = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { tenantId: resolvedTenantId } = requireTenantContext({
                ...
                schema,
                table: 'suppliers',
                operation: 'create supplier from orders module',
            });
            const client = getScopedClient(currentAppSlug);
            ...
            console.error('Error creating supplier from orders module:', error);
