import { getScopedClient } from './supabaseClient';
...
export const scheduleBlocksApi = {
  async listByRange(tenantId: string, range: DateRange, appSlug?: AppSlug) {
    const client = getScopedClient(appSlug);
  ...
  async listHistory(tenantId: string, limit = 100, appSlug?: AppSlug) {
    const client = getScopedClient(appSlug);
    const { data, error } = await client
      .from('schedule_blocks')
      .select('*')
  ...
  async create(tenantId: string, userId: string | null, payload: ScheduleBlockInput, appSlug?: AppSlug) {
    const client = getScopedClient(appSlug);
    const { data, error } = await client
      .from('schedule_blocks')
      .insert({
  ...
  async update(tenantId: string, blockId: string, payload: Partial<ScheduleBlockInput>, appSlug?: AppSlug) {
    const client = getScopedClient(appSlug);
  ...
  async remove(tenantId: string, blockId: string, userId: string | null, appSlug?: AppSlug) {
    const client = getScopedClient(appSlug);
