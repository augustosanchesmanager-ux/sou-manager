...
| D-HOM-12 | **Conta de homologação no tenant Sanchez Barber — CRIADA E VALIDADA (EXECUTADO 2026-08-08, autorização D-HOM-11)** | Provisionada a conta de homologação no tenant produtivo `b716e290...` (Sanchez Barber): `homolog.sanchez@barber.soumanager.com` (id `189053ab-f76b-4e91-90fc-998bb693711d`, role `manager`, status `active`, `onboarding_completed=true`, membership `user_tenants` primária manager, staff auto-criado pelo trigger `36f1705b-...`). **ETAPA B (§8.1#3) DESBLOQUEADA.** Validação de ponta a ponta: login GoTrue `grant_type=password` 200, RLS (`profiles` próprio) OK, `get_auth_access_context` → `tenant_id=b716e290…/manager/active`. **Pitfalls corrigidos (usuário criado via SQL direto):** (1) linha ausente em `auth.identities` (provider `email`); (2) colunas de token (`confirmation_token`, `recovery_token`, `email_change`, `email_change_token_new`, etc.) com `NULL` → causa do `500 Database error querying schema` do GoTrue — corrigido para `''`; (3) hash bcrypt regenerado em custo 10 (padrão GoTrue). Senha **não versionada**. Procedimento completo e evidências: `docs/audit/HOMOLOG_ACCOUNT_PROVISIONING.md`. Falta executar a **validação local no frontend** (login → tenant → Dashboard → Comissões) — autorizada |
