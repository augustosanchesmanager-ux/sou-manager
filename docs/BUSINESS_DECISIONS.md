...
| D-HOM-20 | **H-4 — Billing/Tenant Lifecycle 🟢 APROVADO pelo PO (2026-08-13)** | Decisão do PO: após execução da matriz **H4-1..H4-9: 9/9 PASS** em **tenant E2E isolado** (Supabase real `ushsnmlbeurfvlkieiln`, dados de teste — **NÃO** o tenant Sanchez Barber, conforme D-HOM-19), o gate **H-4 é declarado APROVADO**. Validação via `tests/e2e/homologation/h4-billing-lifecycle.spec.ts` (`E2E_PROVISIONING=1`): H4-1 `active` acesso pleno · H4-2 `past_due` aviso + escrita DB não bloqueada (by design/grace — gap de enforcement na UI registrado) · H4-3 `suspended` bloqueado `/pending-approval` + evento · H4-4 reativação + evento · H4-5 cancelamento `cancel_at_period_end` + efetivação · H4-6 `change_tenant_plan` espelho `tenants.plan`/`subscriptions.plan` + invoice idempotente + payment attempt · H4-7 limites `max_staff` (pro=5/free=1) · H4-8 feature indisponível → `UpgradePrompt` (nunca 403) · H4-9 `runCycle` grace expirado → suspensão + fail-fast. **Falha inicial do H4-8b: corrida de login no teste E2E** (ausência de `waitForURL(/#/dashboard)` após `loginAs`; causa raiz diagnosticada e correção aplicada no spec; reexecução completa **9/9 PASS ~1.0m**) — **NÃO é defeito funcional do produto**. **H-3 permanece 🟡** (H3-1..H3-4, H3-6 ✅; H3-5 🟡 c/ ressalva) até resolução formal da ressalva. **H-8 permanece 🔴 BLOQUEADOR** (produção `718f6f9` + topologia Vercel) — o PASS do H-4 não libera produção. **Sem alteração de código de produção, sem migration, sem merge/tag/deploy.** Próximo gate: **H-5 — Feature Flags**. Registros: `docs/audit/H4_BILLING_LIFECYCLE_VALIDATION.md`, `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-20), `docs/RELEASE_CHECKLIST_v1.5.md` |
