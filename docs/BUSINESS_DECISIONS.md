...
| D-HOM-32 | **H3-5 + P1-01 — PROD HEALTH GATE = PASS; frente ENCERRADA; nenhuma alteração adicional em PROD; 3 pendências como frentes independentes (2026-09-24)** | Veredito formal do PO após post-deploy audit read-only do Production @ `5a2845690dcee121e68427217249c0762d1c56f6`: **PROD HEALTH GATE = PASS → manter `5a28456` em produção.** Evidências aceitas: merge == SHA autorizado; tree == `5ad3c46`; Vercel Production success; `barber.soumanager.com` 200; bundle correto só PROD; rotas OK; **`comanda_items.staff_id` presente e respondendo no PROD**; CI pós-merge obrigatória verde; sem regressão no escopo; **sem fundamento para rollback**; migration `20260924000000_*` **não precisa ser aplicada em PROD**. **Nenhuma alteração adicional em PROD agora.** Cadeia encerrada: change-control/isolate/implement/validate/commit/push/pr/PO review/merge/post-deploy audit = PASS → PROD HEALTHY. **3 pendências — cada uma com próprio `smg-change-control`, NÃO iniciadas nesta decisão, NÃO misturar com esta entrega:** **(P-DV)** `deploy-validate` — condição `deployment.ref == 'refs/heads/main'` vs Vercel `ref=<SHA>` deixa o gate pós-deploy **skipped**; **(P-MIG)** migration `20260924000000_*` — untracked, no-op PROD (coluna existe); fluxo = comparar STAGING × contrato canônico → decidir versionamento → novo gate; **(P-AUTO)** dívida change-control — infra faz `MERGE → push main → Vercel Production`, processo exige `MERGE → PO decide DEPLOY`; hoje “merge sem deploy” não é opção real; correção futura de pipeline, formal como dívida do SMG. **Frente H3-5+P1-01: ENCERRÁVEL.** Próxima frente somente quando o PO escolher qual dívida abrir. Registros: `ROADMAP.md` (8.77), `PROJECT_STATUS.md` (7.38), `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md` (§10.7). |
