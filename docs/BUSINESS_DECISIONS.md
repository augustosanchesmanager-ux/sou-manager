...
| D-HOM-28 | **H-8 — Infraestrutura Vercel/Deployment Topology 🟢 PASS COM CONDIÇÃO (veredito formal do PO, 2026-09-02) + retrofit ACL autorizado em trilha própria** | Revalidação **read-only** (2026-09-02) contra a produção `ushsnmlbeurfvlkieiln`: **premissa D-HOM-10 OBSOLETA** — a produção `smg-barber` está em **`c1c40da`** (FASE 6 completa, deploy automático pós-merge `2ce5a7f`, bundle `/assets/index-DXO2_Rnd.js` validado com código FASE 6 + URL de produção) e as **4 migrations corretivas tenant-scoped JÁ estão aplicadas/verificadas em produção** (assinaturas + guardas `auth.uid()`/`current_tenant_id_from_auth_uid`/membership/superadmin + MD5 `10ae7e89…`/`27e61dc1…`/`faf3aab2…`). **Condição do PASS (ACL residual):** `correct_appointment_attendance`, `register_comanda_payment` e `confirm_appointment_attendance` mantêm em produção `postgres=X,anon=X,authenticated=X,service_role=X` — causa raiz: as migrations corretivas revogam apenas `FROM PUBLIC` (as duas bulk_close revogam `FROM PUBLIC, anon, service_role`); a guarda interna `auth.uid()` mitiga em runtime, mas o least-privilege exige o retrofit. **Decisões do PO:** manter produção `c1c40da` **SIM** · rollback **NÃO** · **criar retrofit das 3 ACLs SIM** · aplicar retrofit imediatamente em produção **NÃO** · **homologar retrofit antes da produção SIM** · `smg-estetica` NÃO bloqueia o D-HOM · **registrar `smg-estetica` para trilha própria SIM**. **Executado (trilha própria):** branch `chore/seguranca-acl-retrofit-3-rpcs` (a partir de `main` `c1c40da`) com a migration `20260902000000_seguranca_fix_acl_tenant_scoped_rpcs.sql` (`REVOKE ALL ... FROM PUBLIC, anon, service_role` + `GRANT EXECUTE TO authenticated` nas 3 RPCs). **H-8 encerrado — não será reaberto.** Observação: `get_comanda_payment_summary` também residual em produção (`anon=X,service_role=X`) — **fora do escopo aprovado**, registrado para trilha própria futura. Próximo bloqueador real: **H-7** (S3-1 R$260 RIOS + janela acompanhada). Registro: `docs/audit/H`
| D-HOM-29 | **H-8 — Retrofit ACL das 3 RPCs: HOMOLOGADO em staging + APLICADO em produção (2026-09-02, PO autorizou)** | **Staging (`tjcvuhynckocmvtqykxp`):** migration `20260902000000_seguranca_fix_acl_tenant_scoped_rpcs.sql` aplicada com sucesso — ANTES: 3 RPCs `postgres=X,anon=X,authenticated=X,service_role=X`; DEPOIS: 3 RPCs `postgres=X,authenticated=X` — `anon`/`service_role` revogados. **Produção (`ushsnmlbeurfvlkieiln`):** PO autorizou a aplicação; migration aplicada com sucesso — ANTES: `postgres=X,anon=X,authenticated=X,service_role=X`; DEPOIS: `postgres=X,authenticated=X` em `confirm_appointment_attendance`, `correct_appointment_attendance`, `register_comanda_payment`. **3/3 PASS.** `get_comanda_payment_summary` mantém `anon=X,service_role=X` — fora do escopo aprovado (trilha futura). **H-8彻底 encerrado — todos os gates de homologação exceto H-7 concluídos.** |OMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-28) |
