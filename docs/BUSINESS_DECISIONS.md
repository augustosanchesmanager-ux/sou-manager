...
| D-HOM-15 | **H-3 — H3-2 (benefício no checkout) ✅ VALIDADO via E2E real (2026-08-13)** | E2E Playwright (conta de homologação `homolog.sanchez@…`, frontend local + banco real `ushsnmlbeurfvlkieiln`): cliente assinante CHEFE EXECUTIVO reconhecido no checkout → modal de crédito → "Aplicar crédito" (item zerado) → fechamento zero-close origem `club_credit` → `/operation-success`. Comanda `80c94c1a...` `paid`/`membership_credit_effect=true`/`payment_method="Club dos Chefes"`/`total=0`; consumo de exatamente 1 crédito do serviço correto (CORTE SIMPLES 4→3; HIDRATAÇÃO intacto); **0 transações** (crédito não gera caixa — ADR-001); 0 erros console/HTTP; 5 avisos cosméticos `src=""` (P3). Evidência: `docs/audit/H3_CHEF_CLUB_BENEFIT_CHECKOUT_VALIDATION.md` |
| D-HOM-17 | **H-3 — H3-5 (permissões) 🟡 VALIDADO COM RESSALVA (2026-08-13)** | Feature `chef_club` ativa para o tenant (`pro`/`active` em `plan_features`; `tenant_has_feature` exige `auth.uid()` — CLI sem sessão retorna `false`); E2E manager acessa `/chef-club-plans`, `/chef-club-subscriptions`, `/chef-club-receivables` (0 erros console/HTTP). **Ressalva:** bloqueio de `barber`/`receptionist` garantido pelo `ManagerRoute` (App.tsx:182) mas **não exercitado via E2E** — o tenant real só possui `manager` e `superadmin`. Recomendado exercitar em tenant de teste. Evidência: `docs/audit/H3_CHEF_CLUB_PERMISSIONS_VALIDATION.md` |
| D-HOM-18 | **H-3 — H3-6 (assinaturas preservadas) ✅ VALIDADO (2026-08-13)** | Conferência SQL read-only contra o snapshot (2026-08-08): **16 assinaturas da baseline preservadas** (3 canceladas: João telles/`0fcde146`, PIETRO/`5db05a4f`, K11/`d8461115`; 13 ativas originais) e **nenhuma perdida/alterada**; total atual 17 (14 ativas + 3 canceladas) — **+1 ativa** = `7b92c958` (cliente de validação HOMOLOG H3 TESTE 2026-08-11, criada na homologação), variação documentada do tenant LIVE. Evidência: `docs/audit/H3_CHEF_CLUB_SUBSCRIPTIONS_VALIDATION.md` |
| D-HOM-19 | **PO AUTORIZA H-4 (Billing/Lifecycle) EM TENANT E2E ISOLADO SEM AGUARDAR VEREDITO FORMAL DO H-3 (2026-08-13)** | Decisão do PO: prosseguir com o gate **H-4** **exclusivamente em tenant de teste E2E isolado e identificável** (não no tenant Sanchez Barber, pois o H-4 é independente do tenant real e não depende de mutação dele); **H-3 permanece 🟡** até resolver/documentar formalmente a ressalva H3-5. **Condições:** não usar Sanchez Barber; provisionar tenant E2E isolado; criar apenas os dados necessários aos cenários; validar criação/ativação, ciclo de cobrança, invoice/receivable, pagamento, renovação, expiração, cancelamento, transições de status, idempotência e jobs/eventos; **não alterar migrations de produção sem nova aprovação**; **parar no ponto de qualquer achado/defeito → diagnóstico antes de corrigir**; **sem merge, tag ou deploy de produção**; entregar matriz H4-* com PASS/FAIL/BLOCKED + recomendação para o próximo gate. Registro: `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-19) |
| D-HOM-21 | **H-5 — Feature Flags ✅ VALIDADO (matriz H5-1..H5-9, 2026-08-13)** | Execução do gate **H-5** em **tenant E2E isolado e identificável** (Supabase real `ushsnmlbeurfvlkieiln`, dados de teste — **NÃO** o tenant Sanchez Barber, D-HOM-19), via `tests/e2e/homologation/h5-feature-flags.spec.ts` (`E2E_PROVISIONING=1`) + inspeção grep. **5/5 testes E2E PASS (54.5s) + H5-8 PASS:** H5-1/2/3 matriz free 14 / pro 15 / premium 20 (coincide com seed `20260806090000` e espelho TS `planCatalog*`) · H5-4 `chef_club` habilitada (pro) → rota/UI liberada · H5-5/6 desabilitada (free via `change_tenant_plan`) → `FeatureUnavailablePage` + `UpgradePrompt` (CTA "Ver Meu Plano"/"Voltar ao Início"), **nunca 403** · H5-7 URL direta `/bi` bloqueada · H5-8 zero leitura direta de `feature_flags` no frontend em runtime — decisão exclusiva via RPC `tenant_has_feature` (`useFeatureFlags`); únicos acessos diretos = adapters DI/teste (`featureOverrideStoreDb`/`planCatalogDb`) sem consumidor runtime; D-6.0.5.3-6 (sem SELECT policy) confirmado · H5-9 override por tenant vence a matriz nos 2 sentidos (premium + `false` bloqueia, `bi` preservada; free + `true` libera). Pontos de atenção da spec (sem alteração de produto): seletores por heading geram falso positivo (`UpgradePrompt` renderiza o título da feature como `<h1>`) e cache de flags por sessão de página (override requer recarga plena). **Sem alteração de código de produção, sem migration, sem merge/tag/deploy.** **H-3 permanece 🟡** e **H-8 permanece 🔴 BLOQUEADOR** — o PASS do H-5 não altera esses status. **Veredito formal do PO proferido em D-HOM-22 (APROVADO).** Próximo gate: **H-6 — Segurança**. Registros: `docs/audit/H5_FEATURE_FLAGS_VALIDATION.md`, `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-21), `docs/RELEASE_CHECKLIST_v1.5.md` |
| D-HOM-22 | **H-5 — Feature Flags 🟢 APROVADO pelo PO (2026-08-13)** | Decisão formal do PO: **o gate H-5 é declarado 🟢 VALIDADO/APROVADO**, com evidências em **tenant E2E isolado** (sem utilização do tenant real Sanchez Barber). **Confirmados pelo PO:** habilitação/desabilitação por feature · comportamento por plano (free 14/pro 15/premium 20) · bloqueio de acesso direto · `FeatureUnavailablePage`/`UpgradePrompt` · **ausência de `403` indevido** · isolamento e override por tenant · **uso exclusivo da RPC `tenant_has_feature` em runtime** · **5/5 cenários E2E + validação estrutural H5-8**. O resultado apresentado é suficiente para fechar o gate **sem misturá-lo com o H-6**. **Não iniciar H-6 na mesma execução.** **H-3 permanece 🟡** (ressalva H3-5). **H-8 permanece 🔴 BLOQUEADOR**. **Sem merge, tag ou deploy de produção.** Registros: `docs/audit/H5_FEATURE_FLAGS_VALIDATION.md` (veredito atualizado), `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-22), `docs/RELEASE_CHECKLIST_v1.5.md` |
| D-HOM-23 | **H-6 — Segurança 🔴 BLOQUEADO — veredito preliminar OpenCode + decisão requerida do PO (2026-08-13)** | Auditoria adversarial **read-only** do gate **H-6** executada pelo OpenCode em **tenants E2E isolados** (A/B/OPS, padrão D-HOM-19; NÃO o tenant Sanchez Barber para mutações) via `tests/e2e/homologation/h6-security.spec.ts` (`E2E_PROVISIONING=1`): **8/8 testes, 39 controles PASS, 9 achados** (F6-A/F6-B anon lê `tenants`/`profiles` — dados reais de produção; F6-2 `close_order` escreve cross-tenant — comanda + estoque; F6-3/F6-4 info disclosure cross-tenant via RPC `tenant_has_feature`/`get_role_permissions`; F6-5/F6-6 `plan_change_requests`/`ticket_messages` sem tenant_id — F6-6 expõe conteúdo real de suporte; F6-7 `kiosk_addons` leitura + escrita cross-tenant; F6-8 usuário suspenso mantém leitura REST). RLS **moderno correto**; falhas concentradas em policies/RPCs **legadas** (root causes em `20260305050000_kiosk_rls_fix.sql`, `20260227223434_fix_all_rls_policies_use_security_definer_function.sql`; RPCs `close_order`/`approve_access_request`/`tenant_has_feature`/`get_role_permissions` sem guarda e fora da revoke anon). **Veredito preliminar OpenCode: 🔴 BLOQUEADO** (P1 com exposição de dado real + escrita cross-tenant). **Nenhum fix automático, sem migration, sem merge/tag/deploy; H-7 não iniciado.** **Decisão requerida do PO:** (1) veredito formal do H-6; (2) aprovação da janela de remediação dos achados (revogar/recriar policies legadas + guardas em RPCs legadas + fechamento de `plan_change_requests`/`ticket_messages`/`kiosk_addons`); (3) autorização para H-7. Evidência: `docs/audit/H6_SECURITY_AUDIT.md` |
| D-HOM-24 | **H-6 — remediação aprovada (P2+F6-1) + regras da janela (2026-08-13)** | Decisão formal do PO: **H-6 🔴 BLOQUEADO (formal)**. **Janela de remediação autorizada — execução item a item; lote NÃO autorizado** (nenhum fix em batch/PR único). **H-7 permanece bloqueado** até a remediação ser aprovada. **Classificação dos 9 achados:** **P0/P1** (F6-A anon lê `tenants`, F6-B anon lê `profiles`, F6-2 `close_order` escrita cross-tenant, F6-6 `ticket_messages` conteúdo real) → correção obrigatória antes de H-7, **execução individual** (proposta detalhada por item em `H6_SECURITY_AUDIT.md` §9.2) · **P2 (F6-3, F6-4, F6-5, F6-7, F6-8) → APROVADOS**: guards de tenant em `tenant_has_feature`/`get_role_permissions`, `plan_change_requests` restrito a superadmin, `kiosk_addons` tenant-scope + revoke anon, `current_tenant_id_from_auth_uid` exige `status='active'` · **F6-1 (P3) → APROVADO**: revoke EXECUTE de anon/PUBLIC em `approve_access_request` **agora**, **sem alterar a lógica**, guarda registrada como **dívida P3** (revisão futura). **Executado:** 6 migrations (`20260813120000`..`20260813120500`), uma por item, commit semântico por item + push. **Aplicação no banco remoto de produção e re-execução E2E da suite H6 requerem aprovação explícita do PO** (regra AGENTS.md). **Escopo em aberto para decisão do PO:** (1) policies irmãs `TO public` do root cause F6-A (`public_select_services`/`clients`/`appointments`/inserts — nunca revogadas nas migrations; não testadas no matrix anon H6) — recomendado incluir no fix F6-A; (2) bloqueio REST por `tenants.status` (suspensão real de tenant não altera `profiles.status`). Evidência: `docs/audit/H6_SECURITY_AUDIT.md` (§9) |
| D-HOM-25 | **H-6 — P0/P1 (F6-A/F6-B/F6-2/F6-6) aprovados e executados (2026-08-13)** | Decisão formal do PO: **aprova os 4 achados P0/P1**, com **F6-A em least-privilege** (não DROP total — `tenants`/`services` têm fluxo anônimo legítimo: kiosk/portal resolvem tenant por slug e listam catálogo público) e **F6-2 por desativação** (sem call site no app; fluxo real usa `finance_settle_comanda`). **Executado:** 4 migrations (`20260813130000`..`20260813130300`), uma por item, commit semântico por item + push — total **10/10 itens** (`20260813120000`..`20260813130300`). **Conteúdo:** F6-A `DROP public_select_tenants`/`public_select_services` + policies anon scoped por `tenants.status` (`active`/`trial`) + column grants mínimos (`tenants(id,name,slug,status)`; `services(id,tenant_id,name,price,duration,active,category)`, com guarda `IF EXISTS` p/ `duration_minutes`/`is_active`) + `REVOKE` anon/PUBLIC; F6-B policy profiles `TO authenticated USING (current_is_super_admin_from_auth_uid())` + revoke anon/PUBLIC; F6-2 `REVOKE EXECUTE` de `close_order(uuid)` a anon/authenticated/PUBLIC + grant service_role; F6-6 `ticket_messages` SELECT/INSERT TO authenticated via JOIN `support_tickets` (tenant/usuario/superadmin) + drop `Users can insert tickets` + revoke anon/PUBLIC. **§9.3 corrigida:** das 5 policies irmãs do root cause de F6-A, **4 já foram dropadas** pelo DO-loop de `20260308_multitenant_hotfix.sql` — só sobreviviam `public_select_tenants` e `public_select_services` (tratadas). **Produto-bug registrado** (fora da remediação, decisão de produto pendente): booking anon do kiosk provavelmente já bloqueado em produção e mismatch `duration_minutes`/`is_active` vs colunas reais (`duration`/`active`) em `services` — kiosk/portal. **Pendente de aprovação explícita do PO:** (1) aplicação remota das 10 migrations; (2) re-execução da suite E2E H6 (reauditoria); (3) liberação do H-7. Evidência: `docs/audit/H6_SECURITY_AUDIT.md` (§9.2/§9.3/§9.5) |
| D-HOM-27b | **H-7 / H2-8 — Decisão do PO sobre os 2 pontos em aberto pós-fechamento do H2-8 (2026-09-02). Registrada após a integração do proof canônico + commit/push + fechamento formal do H2-8.** | O PO registra formalmente as decisões independentes do D-HOM-27 sobre os pontos que permaneciam em aberto após o fechamento do achado H2-8. **(1) `6bd5cbe4` — 🟡 PRESERVAR (Decisão B):** preservar o estado atual em produção; **não realizar correção manual/reconciliação no banco neste momento**; manter o caso como **histórico de teste conhecido**; qualquer futura reconciliação deverá passar por **decisão específica** do PO. **(2) Reversões comissionáveis reais — 🟡 JANELA CONTROLADA (Decisão C):** **não liberar de forma ampla imediatamente**; a reabertura ocorrerá mediante **janela controlada de operação real acompanhada**; a execução deverá ter **observação do comportamento financeiro/comissão** e a **reconciliação pós-operação**; **qualquer anomalia deverá interromper a janela e gerar novo finding/gate, sem workaround direto em produção**. **(3) H2-8 — 🟢 CLOSED (não reabrir):** o gate cumpriu seu objetivo — a cadeia de reversão foi **corrigida** (commit `523192a`), **integrada ao proof canônico** (`tests/homologation/h2-8/h2-8-staging-chain.spec.ts`) e **comprovada ponta a ponta em staging, incluindo idempotência**. **As duas decisões acima permanecem independentes no D-HOM-27 e não alteram o fechamento do H2-8.** **Estado oficial: H2-8 CLOSED.** D-HOM-27 permanece aberto **somente** para a gestão controlada desses dois pontos. **Restrições:** produção **não tocada**; sem deploy/migration; sem workaround em banco. Registros: `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md` (§10.6), `PROJECT_STATUS.md` (7.3), `ROADMAP.md` (8.42) |
| D-HOM-27 | **H-7 — Decisões do PO sobre a operação real (2026-08-14): ambiente dados reais Sanchez Barber + 1 ciclo completo + janela acompanhada + critério de parada + M7 mantida bloqueada; H-7 NÃO autoriza produção** | Decisão formal do PO definindo o roteiro de execução do gate **H-7 — Operação real**: (1) **Ambiente: dados reais do tenant Sanchez Barber** (objetivo do H-7 é validar comportamento financeiro/operacional com dados reais) — **sem apagar, editar ou manipular dados existentes**; o atendimento/comanda do teste deve ser **identificável como homologação**; capturar saldos/contagens relevantes antes e depois; (2) **Escopo: 1 ciclo completo ponta-a-ponta** (H7-1: agenda → atendimento → comanda → pagamento → comissão → fechamento profissional → fechamento caixa → conferência financeira) — ampliar somente se o primeiro ciclo estiver perfeito; (3) **Agendamento: sem execução espontânea** — janela **acompanhada**, idealmente com **Rubens/equipe disponível**; dia/horário definidos pelo PO; (4) **Critério de parada: qualquer divergência financeira, duplicidade, perda de crédito, comissão incorreta, alteração inesperada de saldo ou quebra de fechamento = PARAR imediatamente** — **não corrigir "na hora" no banco**; registrar achado e apresentar ao PO; (5) **M7 permanece BLOQUEADA** — nenhuma migration redundante para "fechar 10/10"; H-6 = 🟢 com **9/10 correções efetivamente necessárias**; M7 = dívida P3 separada (`approve_access_request`); (6) **H-7 NÃO é autorização para produção/deploy** — estado permanece **H-6 🟢 → H-7 ⏳ → H-8 🔴**; sem merge, tag ou deploy. **Executado (doc, sem operação real):** roteiro detalhado + checklist de evidências em `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md` (baseline pré-ciclo B1..B10, ciclo H7-1, quadratura Q1..Q7, controles H2-1..H2-8 + H3-4 + S3 + comissão, critérios de fechamento). **Pendente:** definição da janela acompanhada (dia/horário + equipe) antes de qualquer execução |
| D-HOM-26 | **H-6 — Segurança 🟢 APROVADO COM RESSALVA (veredito formal do PO, 2026-08-14) + H-7 autorizado a abrir (pré-requisitos pendentes)** | Veredito formal do PO: o gate **H-6 é declarado 🟢 APROVADO COM RESSALVA (9/10)** — todas as remediações P0/P1/P2/F6-1 aplicadas e validadas no banco remoto de produção (M1–M6 `120000`..`120400` + M8–M10 `130100`..`130300`, aplicação incremental item a item autorizada por D-HOM-24/D-HOM-25); **reauditoria E2E final 7/7 probes P1–P7 PASS (34.8s) + regressão Sanchez F1–F14 14/14 PASS (45.3s); P0/P1 zero**; canário Sanchez verde em todas as etapas. **M7 (`120500` F6-1) formalmente BLOQUEADA:** efeito (revoke anon/PUBLIC em `approve_access_request`) já existia no banco desde o backup `20260728`; a migration não corrige o vetor real (guarda `auth.uid()`/superadmin) — tratar como **dívida P3 separada** (item próprio, fora da remediação; correção + teste dedicados em etapa posterior). **H-7 — Operação real AUTORIZADO A SER ABERTO**, com **pré-requisitos de execução**: (1) apresentar ao PO a matriz/escopo do gate H-7 (ciclo H7-1: agenda → atendimento → comanda → pagamento → comissão → fechamento profissional → fechamento caixa → conferência) e seus pré-requisitos ANTES de qualquer execução; (2) decisão do PO sobre ambiente: **dados reais do tenant Sanchez Barber** (conforme regra §8.2/§9) **ou** ambiente isolado com conferência SQL; (3) refletir no receivable o crédito Chef Club consumido (H3-4/H-7 pendente) e investigar S3 (10 overdue + 6 pending); (4) evidenciar a quadratura SQL formal da matriz H2-1..H2-8 (cancelamento/reversão) no H-7. **H-3 permanece 🟡** (H3-5 c/ ressalva). **H-8 permanece 🔴 BLOQUEADOR** (produção `718f6f9` + topologia Vercel) — o PASS do H-6 não libera produção. **Sem merge, tag ou deploy de produção.** Registros: `docs/audit/H6_5_PRODUCTION_SAFETY_GATE.md` (§12, veredito atualizado), `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-26), `docs/RELEASE_CHECKLIST_v1.5.md`, `docs/security/SECURITY_AUDIT_RPC.md` (dívida P3 `approve_access_request`) |

| D-HOM-30 | **H-3 — Chef Club 🟢 APROVADO pelo PO + H3-5 🟢 RESOLVIDO (2026-09-23)** | Veredito formal do PO: gate **H-3 declarado 🟢 APROVADO** após resolução da ressalva H3-5 (**Gate C**): E2E `tests/e2e/homologation/h35-managerroute-rbac.spec.ts` em tenant E2E isolado `gatec-h35-*` (STAGING `tjcvuhynckocmvtqykxp`, `E2E_PROVISIONING=1`) — **5/5 PASS**: (1) `barber` bloqueado nas 3 rotas Chef Club via `ManagerRoute`; (2) `receptionist` bloqueado idem; (3) `manager` acesso+conteúdo; (4) `owner`/`superadmin` acesso+conteúdo; (5) feature `chef_club` ativa (`pro`/`active`). **Tenant real Sanchez Barber não utilizado para a mutação** (teardown e2e-seed OK). Validações da subfase: unit **1399 PASS/5 skip**, build exit 0, typecheck exit 0, `git diff --check` limpo. **H-3 deixa de possuir a ressalva H3-5** (H3-1..H3-6 ✅). **Numeração:** identificador formal = **D-HOM-30**; **D-HOM-28/D-HOM-29 preservados como referências históricas órfãs** (citados em `ROADMAP.md` 8.28 — "Fase 4 não-causa-raiz" — e `H10_FOTOGRAFIA_POS_RELEASE_20260816.md` §6 — "análise do hotfix" —, nunca registrados neste registry; sem renumeração): *um identificador de decisão nunca deve ter dois significados* (PO verbatim). **Restrições (PO verbatim):** "Nenhum merge/deploy autorizado por essa decisão" — merge, tag e deploy permanecem **gates separados**; H-8 segue 🔴 BLOQUEADOR. Registros: `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (D-HOM-30), `docs/audit/H3_CHEF_CLUB_PERMISSIONS_VALIDATION.md` (§7), `docs/RELEASE_CHECKLIST_v1.5.md`, `PROJECT_STATUS.md` (7.36), `ROADMAP.md` (8.75), PR #79 |

| D-HOM-31 | **H-7 — P1-01 registrado + execução operacional ENCERRADA na janela 2026-09-23; Q5–Q7 não executados; frente de correção do P1 aberta (2026-09-23)** | Veredito do PO na janela acompanhada 2026-09-23 (Rubens/UI produção + OpenCode read-only): **REGISTRAR P1 → ENCERRAR a execução operacional do H-7 neste ponto → NÃO executar Q5–Q7 → abrir frente específica de correção/validação do P1**; **`6bd5cbe4` permanece preservada, sem mexer nela**. **Contexto:** baseline fresco capturado antes da operação; ciclo validado até Q1–Q4 ✅ (comanda `b5368c28` R$145 cash, tx `d39803f7` única, comissão `7bf99382` HERON R$22,50 active; produto sem comissão = regra); no fechamento profissional o **P1-01 reproduziu-se**: `barber_closing 728307e6` gravou **`status=discrepancy`, `expected_cash=0`, `counted_cash=145`, `cash_difference=145`** (`payment_methods={"Nao informado":45}`), com `barber_closings_complete=true` mesmo em discrepancy. **Causa raiz em 3 níveis da cadeia `payment_method`** (transação grava `'cash'` → comanda `null`→loader `'Nao informado'` → lookup `paymentMethods['Dinheiro']` em `summary.ts:321`/`useCashClosing.ts:494` retorna 0; testes mascarados com fixture `'Dinheiro'`) — **não tratar como simples bug de label**: exige correção/validação **de ponta a ponta** (escrita comanda/transação → loader → fechamento → guarda de UI `countedValue<=0` sem feedback → flag `barber_closings_complete`) antes de considerar o fechamento de caixa confiável. **Impacto: sem perda financeira** (tx/comissão/repasse inalterados); dano = confiabilidade da conferência de caixa. **Nenhuma correção aplicada durante a execução** (regra §2 do roteiro); evidências BEFORE/AFTER preservadas. **H-7 permanece 🔴** até a frente do P1 ser corrigida e validada; depois retomada do H-7 (fechamento de caixa + Q5–Q7) e, em janela própria, os pontos do D-HOM-27b. **H-8 segue 🔴 BLOQUEADOR; sem merge, tag ou deploy.** Registros: `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md` (§10.7, §11), `docs/audit/h7-execution/h7-p1-barber-close-BEFORE-20260923T183533Z.json`, `docs/audit/h7-execution/h7-p1-barber-close-AFTER-20260923T185913Z.json` |

| D-HOM-32 | **H-8 — 🟡 DESBLOQUEADO TECNICAMENTE COM RESSALVAS (veredito formal do PO, 2026-09-26); certificação permanece gate separado** | Veredito formal do PO sobre o gate **H-8 (Infraestrutura Vercel / Deployment Topology)** após a auditoria **B+C read-only** de 2026-09-26 (frente gate B+C, autorizada pelo PO; zero mutações). **H8-1 topologia ✅ · H8-2 domínio oficial → `ushsnmlbeurfvlkieiln` ✅ · H8-3 sem double-deploy ✅ · H8-5 config build ✅ · H8-7 produção atualizada ✅** (deploy `6678953234` = `9aed924`, `success`; premissa `718f6f9` superada; 6/6 deployments `success`; produção `barber.soumanager.com` no projeto oficial `smg-barber`; contexto `sou-manager` legado desconectado desde D-HOM-11; login GoTrue 200; RPC context tenant `b716e290` manager/active; anon bloqueado 401/42501; RLS sem vazamento cross-tenant; reversão SEP leitura OK — `professional_id` inexistente `42703` = drift PROD já conhecido, sem mutação). **Ressalvas metodológicas preservadas:** **H8-4** (env do projeto oficial coerente) e **H8-6** (zero credencial sensível no env) **não revalidados via API Vercel** nesta auditoria (escopo HTTP/GoTrue/RLS/REST; baseline 2026-08-08 mantida; nenhum sintoma funcional) — **não reabrir H8-4/H8-6 por esta razão**. **Escopo desta decisão:** formalização documental do veredito; **NÃO executar mutação de reversão · NÃO corrigir achados · NÃO iniciar P1-01**. **Distinção vinculante:** **"desbloqueado tecnicamente" NÃO equivale à certificação da release** — certificação/homologação é gate separado de decisão do PO (sequência: A-H8 formalização → STOP/revisão PO → certificação/homologação → STOP → P1-01). **Proibições desta frente docs-only:** código, SQL, migration, RPC, RLS, alteração Vercel, alteração de configuração, novo smoke, mutação financeira, certificação antecipada, P1-01 misturado. Registros: `docs/audit/HOMOLOGATION_PLAN_SANCHEZ_BARBER.md` (STATUS/§8/§8.1), `ROADMAP.md` (8.78), `PROJECT_STATUS.md` (7.39) |
