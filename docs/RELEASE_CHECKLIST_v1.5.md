...
- [ ] H-3 Chef Club (adesão, benefícios, regras de plano, reflexos financeiros, permissões) — **🟡 aguardando ratificação do PO — ressalva H3-5 RESOLVIDA (2026-09-23):** H3-1 adesão ✅ · H3-2 benefício no checkout ✅ (E2E real) · H3-3 regras de plano ✅ (E2E real) · H3-4 consumo de créditos ✅ (sem `42703`, idempotência OK, 2 bugs corrigidos na RPC `bulk_close_comandas_with_credits`) · H3-6 assinaturas preservadas ✅ (16 baseline + 1 ativa de teste) · H3-5 permissões ✅ **(Gate C: E2E 5/5 PASS em tenant E2E isolado `gatec-h35-*` — bloqueio `barber`/`receptionist` exercitado via ManagerRoute; manager/owner/superadmin acesso OK; `chef_club` ativa; tenant real intocado)**. **Status geral: 🟡 até ratificação formal do PO** (D-HOM a definir → 🟢) (evidências: `docs/audit/H3_CHEF_CLUB_*.md`, §7)
