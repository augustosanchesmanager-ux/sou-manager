...
> **Atualização 2026-09-23 (ressalva H3-5 RESOLVIDA — Gate C E2E, aguarda ratificação do PO):** a ressalva do H3-5 (bloqueio de `barber`/`receptionist` nunca exercitado via E2E — D-HOM-17/D-HOM-19) foi **exercitada em tenant E2E isolado** via `tests/e2e/homologation/h35-managerroute-rbac.spec.ts` (`E2E_PROVISIONING=1`, STAGING `tjcvuhynckocmvtqykxp`): **5/5 PASS (1.2m, 2026-09-23)** — H35-1/H35-2 bloqueio nas 3 rotas Chef Club (ManagerRoute → `/#/dashboard` ×3); H35-3 `manager` acesso+conteúdo; H35-4 `owner`/`superadmin` acesso+conteúdo; H35-5 feature `chef_club` ativa (`pro`). Tenant `gatec-h35-*` provisionado + teardown e2e-seed — **tenant real Sanchez Barber intocado**. Validações da subfase: unit **1399/5skip** · build exit 0 · typecheck exit 0. Evidência: `docs/audit/H3_CHEF_CLUB_PERMISSIONS_VALIDATION.md` (§7). **H-3 permanece 🟡 até ratificação formal do PO** (D-HOM a definir → 🟢). **Sem merge, tag ou deploy.**

> **Atualização 2026-09-23 (D-HOM-30, veredito do PO — H-3 🟢 APROVADO):** o PO declara o gate **H-3 Chef Club 🟢 APROVADO** e a **H3-5 = 🟢 RESOLVIDO** (Gate C E2E 5/5 PASS em tenant E2E isolado `gatec-h35-*`, STAGING; bloqueio `barber`/`receptionist` via `ManagerRoute` exercitado; `manager`/`superadmin` acesso OK; `chef_club` ativa no plano `pro`; tenant real não utilizado). H-3 deixa de possuir ressalva (H3-1..H3-6 ✅). Identificador formal: **D-HOM-30** (D-HOM-28/29 = referências históricas órfãs preservadas). **Restrições:** merge, tag e deploy permanecem gates separados — **não autorizados por esta decisão**; H-8 segue 🔴. Registro: `docs/BUSINESS_DECISIONS.md` (D-HOM-30).
...
- [x] **H-3 Chef Club — 🟢 APROVADO (D-HOM-30, 2026-09-23):** H3-1 adesão ✅ · H3-2 benefício no checkout ✅ (E2E real) · H3-3 regras de plano ✅ (E2E real) · H3-4 consumo de créditos ✅ (sem `42703`, idempotência OK, 2 bugs corrigidos na RPC `bulk_close_comandas_with_credits`) · H3-6 assinaturas preservadas ✅ (16 baseline + 1 ativa de teste) · H3-5 permissões **🟢 RESOLVIDO** (Gate C: E2E 5/5 PASS em tenant E2E isolado `gatec-h35-*` — bloqueio `barber`/`receptionist` exercitado via ManagerRoute; manager/owner/superadmin acesso OK; `chef_club` ativa; tenant real intocado). **Gate sem ressalva.** A ratificação não autoriza merge, tag ou deploy (gates separados); H-8 segue 🔴 (evidências: `docs/audit/H3_CHEF_CLUB_*.md`, §7; registro: `docs/BUSINESS_DECISIONS.md` D-HOM-30)
...
- [x] **H-7 Operação real — 🟢 APROVADO / ENCERRADO (veredito do PO, 2026-09-25)** (ciclo completo acompanhado: agendamento → atendimento → comanda → pagamento → comissão → fechamentos → conferência) **executado em PROD em 2026-09-25 — Q1–Q7 ✅ PASS, quadratura financeira ok, ausência de duplicidade (roteiro §10.8; 6 JSONs 09-25 em `docs/audit/h7-execution/`). Condições originais (D-HOM-26 — AUTORIZADO A ABRIR; D-HOM-27 — ROTEIRO PRONTO, 2026-08-14):** ambiente = **dados reais do tenant Sanchez Barber** (sem manipular dados existentes; registros identificáveis como homologação; saldos antes/depois) · escopo = **1 ciclo completo H7-1** · agendamento = **janela acompanhada** (Rubens/equipe; dia/horário definidos pelo PO — sem execução espontânea) · **critério de parada = qualquer divergência financeira/duplicidade/perda de crédito/comissão incorreta/queixa de fechamento → PARAR imediatamente, sem corrigir no banco** · **M7 mantida BLOQUEADA** (9/10; dívida P3 separada) · **H-7 NÃO autoriza produção/deploy** (estado: H-6 🟢 → **H-7 🟢** → H-8 🔴). Roteiro + checklist de evidências: `docs/audit/H7_OPERACAO_REAL_ROTEIRO.md`. **Fase 1 (baseline read-only) ✅ 2026-08-16** — `docs/audit/H7_BASELINE_READONLY.md` (clients 302, services 17, products 18/estoque 68, appointments 1.447, comandas 1.384, transactions 736, credits 16 (77 disp/3 usadas), cash_closings 3 draft, barber_closings 0, participantes 377, receivables 47: paid 30/R$ 6.440 · overdue 10/R$ 2.340 · pending 7/R$ 1.360). **Investigação S3 + H3-4 read-only ✅ 2026-08-16** — `docs/audit/H7_1_INVESTIGACAO_S3_READONLY_20260816.md`: **✅ H3-4 reflexo receivable FECHADO** (receivable H3-4 pago via Pix, crédito debitado, sem `42703`/duplicidade) · **🔴 achado S3-1 (P1) duplicidade RIOS** (R$ 260,00 `overdue` duplicado de ciclo já pago — `ON CONFLICT` não disparou por `billing_cycle_end` divergente; **S3 inflado R$ 260,00**; sem correção — tratamento = PO). **Pendências não bloqueadoras:** (1) abertura de caixa registrada após pagamento — desvio de sequência, sem efeito financeiro, correção exige autorização específica do PO; (2) instrumentação B5–B9 de `h7-baseline-capture.mjs` (colunas fantasma) — correção opcional futura. **Ciclo H7-1 EXECUTADO em 2026-09-25 (janela acompanhada; Rubens/UI produção) — gate encerrado 🟢 PASS pelo PO**
  ...
  > **ATUALIZAÇÃO 2026-09-25:** janela **reexecutada** com Rubens (baseline 13:44 → ciclo 13:46–13:48 UTC) — **Q1–Q7 ✅ PASS** (comanda `c304cae5` R$20 cash · tx `eb375af2` · comissão HERON R$10 · `barber_closing 43e97aed` diff 0 · `cash_closing d5efad50` confirmed diff 0); **P1-01 validado de ponta a ponta em produção**; `6bd5cbe4` preservada (janela própria, D-HOM-27b). Aguardava veredito formal do PO (roteiro §9) — **resolvido em 2026-09-25: 🟢 APROVADO / ENCERRADO (nota seguinte)**. Evidências: `docs/audit/h7-execution/` (6 JSONs 09-25) · roteiro §10.8 · `PROJECT_STATUS.md` 7.38 · `ROADMAP.md` 8.77.
  > **VEREDITO 2026-09-25 (decisão do PO): H-7 🟢 APROVADO / ENCERRADO — checkbox `[x]`.** O PO declarou o gate **H-7 Operação real = 🟢 PASS / APROVADO / ENCERRADO**: **execução em PROD** (Rubens via UI produção; OpenCode somente read-only) · **Q1–Q7 ✅ PASS** · **quadratura financeira** (caixa = comanda = transação = R$20; fechamento profissional = comissão = R$10) · **ausência de duplicidade** (1× tx income `eb375af2`; comissão `b462a796` idempotente) · baseline pré/pós estável · comanda real `62873e3c` intocada. **Pendências não bloqueadoras preservadas:** (1) abertura de caixa registrada após pagamento — sem efeito financeiro, **não corrigida nesta tarefa; correção exige autorização específica do PO**; (2) instrumentação B5–B9 de `h7-baseline-capture.mjs` (colunas fantasma → 0) — **não corrigida nesta tarefa; correção opcional futura**. **D-HOM-27b = PENDENTE / janela própria** (`6bd5cbe4` preservada; reversões comissionáveis fechadas). **H-8 permanece 🔴 BLOQUEADOR. Sem merge, tag `v1.5` ou deploy.** Registros: roteiro §1/§9/§11 · `HOMOLOGATION_PLAN` (STATUS/§8.2 Gate H-7) · `PROJECT_STATUS.md` 7.39 · `ROADMAP.md` 8.78.
...
| H-7 janela acompanhada | 🟢 **PASS / ENCERRADO (veredito do PO, 2026-09-25)** — ciclo H7-1 reexecutado 2026-09-25, **Q1–Q7 ✅ PASS** (baseline 13:44 → Rubens UI 13:46–13:48 UTC; P1-01 validado ponta a ponta; quadratura caixa=comanda=tx R$20, fechamento=comissão R$10; ausência de duplicidade; evidências `docs/audit/h7-execution/` 6 JSONs 09-25; roteiro §10.8) — pendências não bloqueadoras: abertura de caixa pós-pagamento · probes B5–B9; `6bd5cbe4` + reversões = janela própria (**D-HOM-27b PENDENTE**) |
