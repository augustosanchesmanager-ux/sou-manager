...
| 2026-08-14 | `20260813130100` (M8) | F6-B `profiles` — policy superadmin sem TO (anon) | pré: `"Superadmins can view all profiles"` roles `{public}` com `((role = ANY('{Super Admin,superadmin}')) OR (id = auth.uid()))` — anon com `auth.uid()` nulo via o filtro completo; grants anon totais em profiles. pós: policy `FOR SELECT TO authenticated USING (current_is_super_admin_from_auth_uid())`; `REVOKE ALL FROM anon/PUBLIC` + `GRANT SELECT,INSERT,UPDATE TO authenticated`; anon **sem grants** (restam authenticated/service_role). `"Users can view own profile"` e `tenant_isolation_profiles_*_v2` intocados | ✅ Aplicada e confirmada |
| 2026-08-14 | — (controle M8) | F6-B REST (anon) | anon `GET /profiles` → **401** (bloqueado); controle sadio: anon `GET /tenants` → **200** 3 linhas (policy `anon_select_active_tenants` intacta) — bloqueio específico de profiles, não falha genérica | ✅ **PASS** — anon negado |
| 2026-08-14 | — (regressão M8) | **F1–F14** regressão Sanchez read-only | `E2E_SANCHEZ_REGRESSION=1` (conta homolog) | ✅ **14/14 PASS (44.9s)** — sem impacto funcional |
| 2026-08-14 | `20260813130200` (M9) | F6-2 `close_order(uuid)` — desativação anon/authenticated/PUBLIC, mantém service_role | pré: acl `{postgres=X, authenticated=X, service_role=X}` — `has_function_privilege`: anon=false · authenticated=true · service_role=true (anon/PUBLIC já sem EXECUTE via revoke de 20260808110000; faltava authenticated). pós: acl `{postgres=X, service_role=X}` — `has_function_privilege`: anon=false · authenticated=**false** · service_role=true · postgres=true. `close_order_with_chef_club` (owner postgres) não afetada (chama internamente) | ✅ Aplicada e confirmada |
| 2026-08-14 | — (controle M9) | probe **P-7** close_order desativado | `--grep "P-7"` (`E2E_PROVISIONING=1`) — REST: `close_order` via authenticated **erro** + via anon **erro** | ✅ **PASS** (29.7s) |
| 2026-08-14 | — (regressão M9) | **F1–F14** regressão Sanchez read-only | `E2E_SANCHEZ_REGRESSION=1` (conta homolog) | ✅ **14/14 PASS (47.7s)** — sem impacto funcional |
| 2026-08-14 | `20260813130300` (M10) | F6-6 `ticket_messages`/`support_tickets` — isolamento por ticket/tenant | pré: `"Users can see messages for their tickets"` USING `true` + `"Users can insert messages"` WITH CHECK `true` (TO public) em ticket_messages; `"Users can insert tickets"` WITH CHECK `true` (TO public) em support_tickets; anon com grants totais. pós: legadas **dropadas**; `ticket_messages_select_v2`/`ticket_messages_insert_v2` (TO authenticated, superadmin OU EXISTS JOIN support_tickets com `tenant_id = current_tenant_id_from_auth_uid()` OR `user_id = auth.uid()`); `tenant_ticket_isolation_v2`/`superadmin_global_visibility` intactas; `REVOKE ALL` anon/PUBLIC (anon **sem grants**) + `GRANT SELECT,INSERT` (ticket_messages) e `SELECT,INSERT,UPDATE` (support_tickets) TO authenticated | ✅ Aplicada e confirmada |
| 2026-08-14 | — (controle M10) | probe **P-6** ticket_messages isolamento | `--grep "P-6"` (`E2E_PROVISIONING=1`) — managerA **não** lê/insere mensagens do ticket do tenantB | ✅ **PASS** (37.9s) |
| 2026-08-14 | — (regressão M10) | **F1–F14** regressão Sanchez read-only | `E2E_SANCHEZ_REGRESSION=1` (conta homolog) | ✅ **14/14 PASS (49.6s)** — sem impacto funcional |
...
**Veredito técnico H-6:** **9/10 remediações aplicadas e validadas** (M1–M6, M8–M10); **M7 formalmente bloqueada por redundância de estado e por não corrigir o vetor real** (guard `auth.uid()` em `approve_access_request` — dívida P3). Canário Sanchez verde em todas as etapas.

**Veredito formal (D-HOM-26, PO 2026-08-14):** **H-6 🟢 APROVADO COM RESSALVA (9/10).** Todas as remediações P0/P1/P2/F6-1 aplicadas e validadas no banco remoto de produção; reauditoria final **P1–P7 7/7 PASS (34.8s)** + **Sanchez F1–F14 14/14 PASS (45.3s)**; P0/P1 zero. **M7 formalmente BLOQUEADA** (efeito já existente no banco desde `20260728`; a migration não corrige o vetor real) → tratada como **dívida P3 separada** (`approve_access_request` guard `auth.uid()`/superadmin — item próprio com correção + teste dedicados). **H-7 — Operação real AUTORIZADO A SER ABERTO**, com pré-requisitos de execução apresentados ao PO (matriz/escopo do ciclo H7-1 + decisão de ambiente dados reais × isolado + reflexo receivable H3-4 + quadratura H2-1..H2-8 + investigação S3). **H-3 🟡** e **H-8 🔴 BLOQUEADOR** permanecem. **Sem merge, tag ou deploy de produção.**
