-- ============================================================
-- PROPOSTA P1.3 — get_dashboard_kpis (artefato de ISOLATE)
-- Arquivo: docs/audit/P1_3_MIGRATION_PROPOSAL_get_dashboard_kpis.sql
-- Status:  PROPOSTA — NÃO APLICAR EM PRODUÇÃO NEM EM STAGING
--          sem autorização explícita do PO (D-P1-04 / smg-change-control).
--
-- Branch:  feature/p1-3-canonical-kpis
-- Decisões incorporadas:
--   D-EST-01 (PO): receita líquida por reversões, preservando o original —
--       Receita = Σ income(paid) − Σ financial_reversals.amount (originais do
--       período); transações de reversão EXCLUÍDAS das despesas.
...
--       atual; excluir cancelled, no_show e antecipados sem atendimento.
--       Base = clientes elegíveis no período anterior.
--
-- ⚠ SQL NÃO testado em staging (acesso read-only autorizado).
--   Revisão manual + testes de equivalência no IMPLEMENT.
-- ============================================================
...
--   Retorno:   JSONB envelope com KPIs canônicos (SEM dimensões analíticas:
--              top services/clients/revenue-by-method ficam fora — "RPC Deus" proibido)
--
-- Segurança:
--   SECURITY DEFINER + gate interno obrigatório:
--     auth.uid() → current_tenant_id_from_auth_uid()
--     → papel permitido (profiles → staff → user_tenants.membership)
--     → escopo de staff (p_staff_id pertence ao tenant)
--     → agregação
--   p_tenant_id NUNCA aceito do frontend (derivado do contexto).
--   SET search_path = public (ADR-021/security audit).
--   Grants: REVOKE PUBLIC/anon + GRANT authenticated (ADR-012).
-- ============================================================
...
BEGIN
  ...
  --   padrão revisado pela auditoria de segurança fase 3.3) ───────────
  ...
  -- ─────────── K1: Receita líq = Σ income(paid, período) − Σ reversões de
  --    originais do período (D-EST-01 — data-base = date do ORIGINAL) ───────────
  ...
  -- K3: Resultado (result_basis = 'transactional' — decisão PO)
  ...
  --    Exclui cancelled, no_show e antecipados sem atendimento (continuam
  --    confirmed/scheduled — nunca 'completed'). ───────────
  ...
  -- ─────────── K9: Performance por profissional (D-PERF-01) — autoridade:
  --    service_execution_participants ∘ comanda_items ∘ comandas.
  --    Atendimento "$count"  = item de comanda PAGA (status paid + financial_effect)
  --      com participante affects_revenue = true.
  --    Receita atribuída     = base do participante (percentage: unit_price×qty
  --      ×payout_value/100; fixed: min(payout_value, unit_price×qty)).
  --    Data-base da execução = COALESCE(closed_at, settled_at, created_at)
  --      (fechamento da comanda; a validar nos testes de equivalência).
  --    ⚠ Dependência: participantes precisam existir p/ itens legados
  --      (backfill_service_execution_participants) — validar dados reais.
  ...
  -- ─────────── Retorno: envelope JSONB (KPIs canônicos, sem dimensões) ───────────
