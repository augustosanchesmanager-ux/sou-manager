...
>
> **Atualização 2026-08-14 (aplicação incremental M1+M2+M3):** **M1 (`20260813120000`, F6-3)** aplicada e confirmada (corpo de `tenant_has_feature` exige `p_tenant_id = current_tenant_id_from_auth_uid()` OR superadmin; fail-closed `false`). **M2 (`20260813130000`, F6-A)** aplicada e confirmada: `public_select_services` dropada; `anon_select_active_tenants`/`anon_select_services_active_tenant` (scoped por status); column grants mínimos ao anon; `authenticated` intacto. **M3 (`20260813120100`, F6-4)** aplicada e confirmada: `get_role_permissions` com guarda de ownership (`current_is_super_admin_from_auth_uid() OR current_tenant_id_from_auth_uid() = p_tenant_id`, senão `RAISE`); somente a RPC alterada (assinatura/volatility/grants intactos). **Probes PASS:** P-1 (cross-tenant services), P-4 (RPC protegida F6-3+F6-4). **Regressão Sanchez F1–F14: 14/14 PASS** pós-M2 e pós-M3. **Canário corrigido (só spec):** `ticket_messages` usa `sender_id`, não `user_id`. Sem impacto funcional no app da Sanchez. Aguardando veredito do PO para os próximos itens (`120200`..`130300`). Registro completo: `H6_5_PRODUCTION_SAFETY_GATE.md` §12.
>
