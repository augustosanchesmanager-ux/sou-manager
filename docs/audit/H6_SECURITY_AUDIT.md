...
>
> **Atualização 2026-08-14 (aplicação incremental M1..M4):** **M1 (`20260813120000`, F6-3)** e **M2 (`20260813130000`, F6-A)** aplicadas e confirmadas (detalhes no commit anterior). **M3 (`20260813120100`, F6-4)** aplicada: `get_role_permissions` com guarda de ownership (superadmin OR tenant do chamador, senão `RAISE`). **M4 (`20260813120200`, F6-5)** aplicada: `plan_change_requests` restrito a superadmin — policies legadas dropadas, criadas `superadmin can view/insert plan requests` (SELECT/INSERT TO authenticated com superadmin); **controle REST com a conta homolog (não-superadmin): SELECT 1 linha → 0 linhas e INSERT rejeitado por RLS**. **Probes PASS:** P-1, P-4. **Regressão Sanchez F1–F14: 14/14 PASS** após cada item (48.4s / 44.1s / 45.4s). **Canário corrigido (só spec):** `ticket_messages` usa `sender_id`. Sem impacto funcional no app da Sanchez. Aguardando veredito do PO para os próximos itens (`120300`..`130300`). Registro completo: `H6_5_PRODUCTION_SAFETY_GATE.md` §12.
>
