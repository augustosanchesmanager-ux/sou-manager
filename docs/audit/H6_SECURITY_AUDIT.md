...
>
> **Atualização 2026-08-14 (aplicação incremental M1..M5):** **M1 (`20260813120000`, F6-3)**, **M2 (`20260813130000`, F6-A)**, **M3 (`20260813120100`, F6-4)** e **M4 (`20260813120200`, F6-5)** aplicadas e confirmadas (detalhes nos commits anteriores). **M5 (`20260813120300`, F6-7)** aplicada: `kiosk_addons` com policies tenant-scoped (`tenant_id = current_tenant_id_from_auth_uid()` OR superadmin) + `REVOKE ALL` anon/PUBLIC; **controle REST**: anon `permission denied` (pré-M5: 1 linha visível), manager mantém leitura própria, INSERT cross-tenant rejeitado por RLS. **Probes PASS:** P-1, P-4. **Regressão Sanchez F1–F14: 14/14 PASS** após cada item (48.4s / 44.1s / 45.4s / 51.3s). **Canário corrigido (só spec):** `ticket_messages` usa `sender_id`. Sem impacto funcional no app da Sanchez. Aguardando veredito do PO para os próximos itens (`120400`..`130300`). Registro completo: `H6_5_PRODUCTION_SAFETY_GATE.md` §12.
>
