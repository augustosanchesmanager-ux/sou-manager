...
> **Progresso H-3 (2026-08-13):** **H3-4 (reflexos financeiros — créditos) em validação via SQL** — consumo de créditos do Clube validado no banco real contra o cenário do erro `42703` histórico (ver `H3_CHEF_CLUB_CREDITS_VALIDATION.md`): exatamente 1 crédito consumido (5→4), comanda fechada com `membership_credit_effect=true` sem `42703`, cenário negativo `P0001` sem `42703`, idempotência OK. **2 bugs de runtime corrigidos na RPC** `bulk_close_comandas_with_credits` (precedência do cast `::jsonb` + `jsonb_set create_missing=false`). H3-1/H3-2/H3-3/H3-5/H3-6 pendentes.
...
3. **H-3 Chef Club em execução (2026-08-13):** H3-4 (consumo de créditos via RPC `bulk_close_comandas_with_credits`) validado no banco real — exatamente 1 crédito (5→4), sem `42703`, idempotência OK, 2 bugs de runtime corrigidos na RPC. **Pendentes:** H3-1 adesão, H3-2 benefício no checkout, H3-3 regras de plano, H3-5 permissões, H3-6 conferência das 16 assinaturas, reflexo no receivable (H-7).
