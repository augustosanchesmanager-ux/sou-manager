...
> **Atualização 2026-08-08 (D-HOM-12):** **conta de homologação CRIADA e VALIDADA** no tenant Sanchez Barber — `homolog.sanchez@barber.soumanager.com` (id `189053ab-f76b-4e91-90fc-998bb693711d`, role `manager`, `active`, membership primária, staff auto-criado pelo trigger). Login GoTrue 200, RLS OK e `get_auth_access_context` → `b716e290...`/manager/active. **ETAPA B (§8.1#3) DESBLOQUEADA.** Durante o provisionamento foram identificados e corrigidos 2 requisitos do GoTrue para usuários criados via SQL (linha em `auth.identities` + colunas de token `*_token`/`email_change` em `''`, não `NULL` — causa do `500 Database error querying schema`). Procedimento completo: `docs/audit/HOMOLOG_ACCOUNT_PROVISIONING.md`.
...
| 3 | Auth / TenantContext (ETAPA B) | Repro do `Invalid Refresh Token` no frontend oficial → logout/limpeza de sessão → login → conferir `auth.uid()`, resolução de tenant (`get_auth_access_context`), Dashboard e Comissões; se `TenantContext` seguir falhando → registrar P1/P0 (independente do fix de comissões) | ✅ **CONTA DE HOMOLOGAÇÃO CRIADA E VALIDADA (D-HOM-12, 2026-08-08)** — `homolog.sanchez@barber.soumanager.com` (manager/active, membership primária, staff via trigger); login GoTrue 200 + RLS + `get_auth_access_context` OK. **Falta executar a validação local no frontend** (login → tenant → Dashboard → Comissões) | Validação autorizada (conta pronta) |
...
> **Pré-condição global:** conta de homologação no tenant Sanchez Barber (ETAPA B) **✅ CRIADA E VALIDADA (D-HOM-12)** + re-teste de Comissões sobre o preview oficial `smg-barber` (§8.1#4/#5) — **pendente** (execução local da ETAPA B e re-teste).
...
2. **Pré-homologação (autorizada 2026-08-08):** snapshot/baseline concluído (`SNAPSHOT_PRE_HOMOLOGACAO_SANCHEZ_BARBER_v1_5_0.md`) + especificação de execução H-1..H-7 (§8.2) pronta + **conta de homologação no tenant CRIADA E VALIDADA (D-HOM-12)** — **ETAPA B desbloqueada**; falta executar a validação local no frontend (login → tenant → Dashboard → Comissões).
