...
| 3 | Auth / TenantContext (ETAPA B) | Repro do `Invalid Refresh Token` no frontend oficial → logout/limpeza de sessão → login → conferir `auth.uid()`, resolução de tenant (`get_auth_access_context`), Dashboard e Comissões; se `TenantContext` seguir falhando → registrar P1/P0 (independente do fix de comissões) | ⏳ **ADIADA (PO, 2026-08-08)** — aguardando conta de homologação no tenant Sanchez Barber (não existe conta de teste; `profiles` só tem o superadmin; `user_tenants` vazio) | Validação autorizada (falta a conta) |
