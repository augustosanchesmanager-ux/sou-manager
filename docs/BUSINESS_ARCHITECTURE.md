...
| 2 | Planos oficiais (nome, valor, créditos, duração) | ✅ Definido (Free, Pro, Elite) |
| 3 | Grace period após vencimento | ⬜ Pendente |
...
|-------|---------|--------|
| Planos | `free`, `pro`, `elite` | ✅ Definido no DB |
| Feature flags | ❌ Não implementado | ⬜ |
| Limites por plano | Configuráveis (sem valores fixos) | ✅ Definido |
...
| **Elite** | Plano premium. Destinado a empresas que desejam recursos avançados, BI, automações e futuras integrações. | ✅ Definido |
...
| # | Item | Status |
|---|------|--------|
| 1 | Nomes dos planos | ✅ Definido (Free, Pro, Elite) |
| 2 | Preços dos planos | ⬜ Pendente (estratégia comercial) |
| 3 | Funcionalidades por plano | ✅ Definido (catálogo em SAAS_CORE_ARCHITECTURE.md) |
| 4 | Limites por plano | ✅ Definido (configurável, sem valores fixos) |
| 5 | Período de trial | ⬜ Pendente |
| 6 | Regras de upgrade/downgrade | ⬜ Pendente |
...
|-------|------|---------|----------|
| `tenants.active` | `boolean \| null` | true/false/null | tenant.ts |
| `tenants.plan` | `string` | free/pro/elite | tenant.ts |
| `profiles.status` | `string` | pending/active/suspended | AuthContext.tsx |

### 9.2 Estados Oficiais (Decisão do PO — 2026-07-28)
...
| trial | active | Assinatura ativada |
| active | past_due | Pagamento vencido |
| past_due | active | Pagamento confirmado |
| past_due | suspended | Grace period expirado |
| suspended | active | Pagamento confirmado (reativação) |
| suspended | cancelled | Cancelamento solicitado |
| active | cancelled | Cancelamento solicitado |
| cancelled | archived | Período de retenção expirado |
...
| 2 | Grace period padrão | ⬜ Pendente |
| 3 | O que acontece com dados ao cancelar | ⬜ Pendente |
| 4 | Período de retenção de dados | ⬜ Pendente |
| 5 | Processo de reativação | ⬜ Pendente |
...
| 2 | Lifecycle: draft → trial → active → past_due → suspended → cancelled → archived | 9 |
| 3 | Planos: Free, Pro, Elite. Sem preços na documentação | 8 |
| 4 | Limites: configuráveis por plano, sem valores fixos | 8 |
...
| 2 | Definir grace period e período de retenção de dados | 9 |
| 3 | Definir gateway de pagamento (futuro) | 6 |
