...
const Suppliers: React.FC = () => {
  ...
  const filtered = useMemo(
    () =>
      suppliers.filter((supplier) =>
        `${supplier.name} ${supplier.email || ''} ${supplier.phone || ''} ${supplier.category || ''}`
          .toLowerCase()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await client
        .from('suppliers')
        .insert([{ ...form, tenant_id: resolvedTenantId }]);
      ...
      setToast({
        message: editingSupplier ? 'Erro ao atualizar fornecedor.' : 'Erro ao cadastrar fornecedor.',
        type: 'error',
      });
    }
  };
  ...
  const handleDelete = async (supplierId: string) => {
    try {
      ...
      setToast({ message: 'Fornecedor excluido.', type: 'info' });
      void fetchSuppliers();
    } catch (error) {
    ...
    <div className="space-y-6 animate-fade-in">
                ...
                filtered.map((supplier) => (
                  <tr key={supplier.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <p className="text-[11px] text-slate-500">{supplier.address || 'Sem endereco cadastrado'}</p>
                    </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold uppercase text-slate-500">Endereco</label>
            <input
              value={form.address}
