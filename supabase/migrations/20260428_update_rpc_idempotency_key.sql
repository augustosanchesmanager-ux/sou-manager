...
BEGIN
  ...
  INSERT INTO public.appointments (tenant_id, client_id, service_id, staff_id, client_name, client_phone, service_name, staff_name, start_time, duration, notes, status, idempotency_key)
  VALUES (p_tenant_id, p_client_id, p_service_id, p_staff_id, v_client_name, NULLIF(BTRIM(p_client_phone), ''), v_service_name, v_staff_name, p_start_time, v_duration_hours, NULLIF(BTRIM(p_notes), ''), 'confirmed', p_idempotency_key)
  RETURNING id INTO v_appointment_id;
  INSERT INTO public.comandas (tenant_id, appointment_id, client_id, staff_id, status, total, idempotency_key)
  VALUES (p_tenant_id, v_appointment_id, p_client_id, p_staff_id, 'open', v_service_price, p_idempotency_key)
...
GRANT EXECUTE ON FUNCTION public.create_appointment_with_comanda(UUID, UUID, TEXT, TEXT, UUID, UUID, TIMESTAMPTZ, NUMERIC, TEXT, TEXT) TO authenticated;
