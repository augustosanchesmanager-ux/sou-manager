...
-- Auditable stock settlement for comanda checkout.
-- Prepared locally on 2026-06-09.
-- Do not apply to remote without explicit approval.
-- This migration intentionally creates public.inventory_movements and ignores
-- control.inventory_movements, which has a different structure and lifecycle.

DO $preflight$
BEGIN
  IF to_regclass('public.inventory_movements') IS NOT NULL THEN
    RAISE EXCEPTION 'public.inventory_movements already exists; review existing ledger before applying this migration';
  END IF;

  IF to_regprocedure('public.apply_inventory_sale_for_comanda(uuid, uuid, text, uuid)') IS NOT NULL THEN
    RAISE EXCEPTION 'public.apply_inventory_sale_for_comanda already exists; review current function before applying this migration';
  END IF;

  IF to_regprocedure('public.finance_settle_comanda(uuid, uuid, text, numeric, timestamp with time zone, text, text, text)') IS NULL THEN
    RAISE EXCEPTION 'Expected finance_settle_comanda signature not found';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'products.stock_quantity is required for auditable stock settlement';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'comanda_items' AND column_name = 'product_id'
  ) THEN
    RAISE EXCEPTION 'comanda_items.product_id is required for auditable stock settlement';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'comanda_items' AND column_name = 'quantity'
  ) THEN
    RAISE EXCEPTION 'comanda_items.quantity is required for auditable stock settlement';
  END IF;
END;
$preflight$;
...
CREATE UNIQUE INDEX idx_inventory_movements_tenant_idempotency
...
BEGIN
  ...
  IF p_user_id IS NULL THEN
    RAISE EXCEPTION 'user_id obrigatorio para baixa de estoque';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Item de comanda com produto sem tenant_id. Corrija o dado antes de liquidar.';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE ci.comanda_id = p_comanda_id
      AND ci.product_id IS NOT NULL
      AND ci.tenant_id IS DISTINCT FROM p_tenant_id
  ) THEN
    RAISE EXCEPTION 'Item de comanda com produto pertence a outro tenant. Liquidação bloqueada.';
  END IF;
  ...
  LOOP
    ...
    INSERT INTO public.inventory_movements (
      ...
      jsonb_build_object(
        ...
        'source_idempotency_key', NULLIF(BTRIM(COALESCE(p_idempotency_key, '')), ''),
...
-- Recreated from the remote finance_settle_comanda definition captured before
-- stock settlement, with only the stock-settlement call inserted after the
-- comanda lock/status checks and before financial mutation.
CREATE OR REPLACE FUNCTION public.finance_settle_comanda(p_tenant_id uuid, p_comanda_id uuid, p_payment_method text, p_paid_amount numeric, p_payment_date_real timestamp with time zone DEFAULT now(), p_source text DEFAULT 'checkout'::text, p_notes text DEFAULT NULL::text, p_idempotency_key text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
