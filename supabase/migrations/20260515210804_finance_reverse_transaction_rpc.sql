...
-- Fase 5C - RPC transacional para estornos, reversoes e devolucoes.
...
BEGIN
  ...
  IF NULLIF(BTRIM(COALESCE(p_reversal_type, '')), '') IS NULL THEN RAISE EXCEPTION 'Tipo de reversao obrigatorio'; END IF;
  IF NULLIF(BTRIM(COALESCE(p_reason_type, '')), '') IS NULL THEN RAISE EXCEPTION 'Motivo obrigatorio'; END IF;
  ...
  IF v_key IS NOT NULL THEN
    SELECT fr.id, fr.reversal_transaction_id INTO v_reversal_id, v_reversal_transaction_id
  ...
  VALUES (p_tenant_id, v_auth_uid, 'expense', CASE WHEN v_original.source_type = 'comanda' THEN 'Estorno de Comanda' ELSE 'Estorno Financeiro' END, 'Reversao da transacao ' || p_original_transaction_id::text, p_amount, COALESCE(v_refund_method, v_original.payment_method), v_reversal_date, 'paid', v_reason_note, v_original.source_type, v_original.source_id, v_key,
    jsonb_build_object('original_transaction_id', p_original_transaction_id, 'reversal_type', p_reversal_type, 'reason_type', p_reason_type, 'reason_note', v_reason_note, 'available_before', v_available_amount))
  ...
  VALUES (p_tenant_id, p_original_transaction_id, v_reversal_transaction_id, v_original.source_type, v_original.source_id, p_reversal_type, p_amount, p_reason_type, v_reason_note, v_refund_method, v_key, v_auth_uid,
    jsonb_build_object('original_amount', v_original.amount, 'available_before', v_available_amount))
  RETURNING id INTO v_reversal_id;

  IF v_original.source_type = 'comanda' AND p_reversal_type = 'wrong_settlement' AND p_amount >= COALESCE(v_original.amount, 0) THEN
