...
import { updateSubscriptionStatus } from '../application/chefClub';
...
const ChefClubSubscriptions: React.FC = () => {
    ...
    const translateStatus = (status: SubscriptionStatus | 'all') => subscriptionStatusLabels[status] || status;
    ...
    const handleWhatsApp = (phone: string, name: string) => {
        ...
        const message = encodeURIComponent(
            `Olá ${name}! Passando para lembrar da sua assinatura do Club dos Chefes. Para manter seus créditos e benefícios ativos, podemos regularizar por aqui.`,
        );
    ...
    const handleStatusChange = async (subscriptionId: string, newStatus: 'paused' | 'canceled' | 'active') => {
        ...
        try {
            await updateSubscriptionStatus(tenantId, subscriptionId, newStatus);

            const actionLabel = subscriptionActionLabels[newStatus] || newStatus;
                ...
                <div className="grid gap-5 p-5 md:grid-cols-[1fr_auto] md:items-end md:p-6">
                    <div className="max-w-3xl">
                        <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold text-[#BFEFFF]">
                            ...
                            Club dos Chefes, recorrência da barbearia
                        </div>
