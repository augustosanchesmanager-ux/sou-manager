...
const tabs: { id: AdminTab; label: string; icon: React.ReactNode }[] = [
  { id: 'overview', label: 'Visão Geral', icon: <HandCoins className="h-4 w-4" /> },
  { id: 'companies', label: 'Empresas', icon: <Building2 className="h-4 w-4" /> },
{ id: 'users', label: 'Usuários', icon: <Users className="h-4 w-4" /> },
    { id: 'subscriptions', label: 'Solicitações', icon: <CreditCard className="h-4 w-4" /> },
...
const SuperAdmin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('Não foi possível restaurar a visão salva do SuperAdmin.', error);
    }
  }, []);
  ...
  const subscriptions = useMemo<SubscriptionRequestRow[]>(() => plans.map((plan) => {
    ...
    return {
      id: plan.id,
      company: tenant?.name || 'Sem tenant',
      owner: profile?.full_name || 'Usuário sem nome',
      ...
      history: [`Atual: ${plan.current_plan || 'Não informado'}`, `Solicitado: ${plan.requested_plan || 'Não informado'}`],
    };
  ...
  const activities = useMemo<AdminActivity[]>(() => {
    ...
    tickets.forEach((ticket) => {
      ...
      rows.push({
        ...
        actor: profile?.full_name || 'Usuário',
    ...
    accessRequests.forEach((request) => {
      rows.push({
        ...
        summary: `Solicitação recebida para ${request.tenant_name || 'empresa nova'}.`,
        eventType: 'access_request',
        notes: request.phone || request.email || 'Sem contato adicional.',
        technicalLog: JSON.stringify(request, null, 2),
        auditTrail: [`Status: ${request.status}`, `Email: ${request.email || 'Não informado'}`],
      });
    });
  ...
  const kpis = useMemo<AdminKpi[]>(() => {
    ...
    return [
      { id: 'kpi-tenants', title: 'Empresas ativas', value: String(tenants.filter((tenant) => tenant.status === 'active').length), delta: `${tenants.length} tenants cadastrados`, context: 'Base real de tenants em operação.', icon: Building2, tone: 'gold' },
      { id: 'kpi-users', title: 'Usuários mapeados', value: String(profiles.length), delta: `${profiles.filter((profile) => (profile.role || '').toLowerCase() === 'manager').length} gestores`, context: 'Perfis reais encontrados em profiles.', icon: Users, tone: 'emerald' },
      { id: 'kpi-requests', title: 'Solicitações pendentes', value: String(plans.filter((plan) => plan.status === 'pending').length + accessRequests.filter((request) => request.status === 'pending').length), delta: `${tickets.filter((ticket) => ticket.status !== 'closed').length} tickets em aberto`, context: 'Fila comercial e operacional atual.', icon: CreditCard, tone: 'blue' },
      { id: 'kpi-alerts', title: 'Alertas críticos', value: String(alerts.filter((alert) => !alert.resolved_at && alert.level === 'critical').length), delta: `${alertItems.length} alertas ativos`, context: 'Incidentes reais do ambiente.', icon: AlertTriangle, tone: 'red' },
      { id: 'kpi-finance', title: 'Movimento financeiro', value: formatCurrency(income), delta: `${transactions.length} transações lidas`, context: 'Receitas registradas em transactions.', icon: HandCoins, tone: 'gold' },
      { id: 'kpi-audit', title: 'Eventos auditados', value: String(auditLogs.length), delta: `${auditLogs.filter((audit) => audit.action === 'DELETE').length} exclusões`, context: 'Trilha real vinda de audit_logs.', icon: ScrollText, tone: 'slate' },
    ];
  }, [accessRequests, alertItems.length, alerts, auditLogs, plans, profiles, tenants, tickets, transactions]);

  const filterOptions = useMemo(() => ({
    periods: [{ value: '24h', label: 'Últimas 24h' }, { value: '7d', label: 'Últimos 7 dias' }, { value: '30d', label: 'Últimos 30 dias' }, { value: '90d', label: 'Últimos 90 dias' }],
    ...
    movementTypes: [{ value: 'all', label: 'Todas movimentações' }, ...Array.from(new Set(activities.map((item) => item.type))).map((type) => ({ value: type, label: type }))],
    plans: [{ value: 'all', label: 'Todos planos' }, ...Array.from(new Set(subscriptions.map((item) => item.plan))).map((plan) => ({ value: plan, label: plan }))],
    users: [{ value: 'all', label: 'Todos usuários' }, ...users.map((user) => ({ value: user.name, label: user.name }))],
    financialStates: [{ value: 'all', label: 'Toda situação financeira' }, { value: 'regular', label: 'Regular' }, { value: 'inadimplente', label: 'Inadimplente' }, { value: 'aguardando pagamento', label: 'Aguardando pagamento' }],
  ...
  const quickActions: QuickAction[] = [
    ...
    { id: 'go-users', label: 'Revisar usuários', description: 'Abre a lista real de perfis.', tone: 'default' },
    { id: 'go-subscriptions', label: 'Ver solicitações', description: 'Mostra os pedidos de plano ativos.', tone: 'default' },
  ...
  const handleExport = () => {
    ...
    link.download = `administração-geral-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  const handleSaveView = () => {
    ...
    setToast({ message: 'Visão salva neste navegador.', type: 'success' });
  };
  ...
  const companyColumns: DataColumn<CompanyRow>[] = [
    ...
    { key: 'usersCount', label: 'Usuários', render: (item) => item.usersCount },
  ...
  const userColumns: DataColumn<ManagedUserRow>[] = [
    { key: 'name', label: 'Usuário', render: (item) => <div><p className="font-bold text-slate-950 dark:text-white">{item.name}</p><p className="text-xs text-slate-500 dark:text-slate-400">{item.id}</p></div> },
  ...
  const subscriptionColumns: DataColumn<SubscriptionRequestRow>[] = [
    ...
    { key: 'action', label: 'Ação', align: 'right', render: (item) => <div className="flex justify-end"><Button variant="secondary" size="sm" className="rounded-xl" onClick={() => setToast({ message: `Solicitação ${item.id} aberta para análise.`, type: 'info' })}>Ver histórico</Button></div> },
  ];
  const auditColumns: DataColumn<AuditEntry>[] = [
    ...
    { key: 'action', label: 'Ação', render: (item) => item.action },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
          <div className="space-y-6 xl:col-span-8">
            ...
            <DataTable title="Solicitações de plano" description="Pedidos reais de mudança de plano no ambiente." columns={subscriptionColumns} rows={pagedSubscriptions.items} page={subscriptionPage} totalPages={pagedSubscriptions.totalPages} onPageChange={setSubscriptionPage} emptyTitle="Nenhuma solicitação encontrada" emptyDescription="Não há mudanças de plano para os filtros atuais." />
            <DataTable title="Usuários da plataforma" description="Perfis reais encontrados em profiles." columns={userColumns} rows={pagedUsers.items} page={userPage} totalPages={pagedUsers.totalPages} onPageChange={setUserPage} emptyTitle="Nenhum usuário localizado" emptyDescription="Não há usuários para os filtros aplicados." />
            <DataTable title="Auditoria administrativa" description="Trilha real vinda de audit_logs." columns={auditColumns} rows={filtered.audits.slice(0, 6)} page={1} totalPages={1} onPageChange={() => undefined} emptyTitle="Nenhum evento auditado" emptyDescription="Não há eventos auditados para os filtros atuais." />
          </div>
      ...
      {activeTab === 'companies' && <DataTable title="Empresas e tenants" description="Base real de tenants cadastrados." columns={companyColumns} rows={pagedCompanies.items} page={companyPage} totalPages={pagedCompanies.totalPages} onPageChange={setCompanyPage} emptyTitle="Nenhuma empresa encontrada" emptyDescription="Não há tenants para os filtros atuais." />}
      {activeTab === 'users' && <DataTable title="Usuários da plataforma" description="Perfis reais encontrados em profiles." columns={userColumns} rows={pagedUsers.items} page={userPage} totalPages={pagedUsers.totalPages} onPageChange={setUserPage} emptyTitle="Nenhum usuário encontrado" emptyDescription="Não há perfis correspondentes aos filtros." />}
      {activeTab === 'subscriptions' && <DataTable title="Solicitações de plano" description="Mudanças reais de plano vindas de plan_change_requests." columns={subscriptionColumns} rows={pagedSubscriptions.items} page={subscriptionPage} totalPages={pagedSubscriptions.totalPages} onPageChange={setSubscriptionPage} emptyTitle="Nenhuma solicitação encontrada" emptyDescription="Não há solicitações no período." />}
      {activeTab === 'audit' && <DataTable title="Auditoria administrativa" description="Registros reais da tabela audit_logs." columns={auditColumns} rows={filtered.audits} page={1} totalPages={1} onPageChange={() => undefined} emptyTitle="Nenhum evento auditado" emptyDescription="Não há eventos auditados para os filtros atuais." />}
